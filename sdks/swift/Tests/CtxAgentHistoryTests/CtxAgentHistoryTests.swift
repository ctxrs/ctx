...
final class CtxAgentHistoryTests: XCTestCase {
    ...
    func testStructuredErrors() throws {
        ...
        let oversized = LocalCLIAdapter(runner: CapturingRunner { _ in
            CommandResult(stdout: Data(repeating: 0x78, count: 2 * 1024 * 1024 + 1))
        })
        XCTAssertThrowsError(try oversized.execute(["status"])) { error in
            let sdkError = error as? CtxAgentHistorySDKError
            XCTAssertEqual(sdkError?.code, .captureLimit)
            XCTAssertNil(sdkError?.stdout)
            XCTAssertNil(sdkError?.stderr)
        }
    }
