...
pub fn resolve_ctx_mcp_command_for_test() -> Option<PathBuf> {
    option_env!("CARGO_BIN_EXE_ctx-mcp").map(|raw_path| resolve_cargo_bin_exe(raw_path))
}

pub fn ctx_mcp_command_env_pair() -> Option<(String, String)> {
    Some((
...
pub fn set_ctx_mcp_command_env_for_test() -> Option<TestEnvGuard> {
    Some(TestEnvGuard::set(
        "CTX_MCP_COMMAND",
        resolve_ctx_mcp_command_for_test()?,
    ))
}
...
pub async fn provider_scenarios_offline_daemon_fixture(
    ...
    let (runtime_command, runtime_args) =
        crp_fixture_runtime::fixture_runtime_invocation(python, &script_path);
    seed_managed_codex_cli_host_runtime_with_args(data_dir.path(), &runtime_command, runtime_args)
        .await;
    let providers =
