...
mod tests {
    ...
    async fn auth_event_sink_attaches_order_seq_selectively_and_counts_legacy_init_key() {
        ...
        assert!(summary.metrics.iter().any(|metric| {
            metric.labels.get("surface").map(String::as_str) == Some("sessions.auth_event_init")
                && metric.labels.get("issue").map(String::as_str) == Some("crp_session_id")
        }));
    }
    ...
    async fn session_control_interrupt_records_root_kind_telemetry() {
        ...
        assert!(summary.metrics.iter().any(|metric| {
            metric.labels.get("provider_id").map(String::as_str) == Some("fake")
                && metric.labels.get("model_id").map(String::as_str) == Some("model")
                && metric
                    .labels
                    .get("execution_environment")
                    .map(String::as_str)
                    == Some("host")
                && metric.labels.get("session_root_kind").map(String::as_str) == Some("worktree")
                && metric.labels.get("event").map(String::as_str) == Some("http_dispatch")
        }));
    }
