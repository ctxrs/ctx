...
#[cfg(target_os = "macos")]
fn macos_direct_replace_permission_denied(path: &Path, err: std::io::Error) -> Error {
    Error::Io(std::io::Error::new(
        std::io::ErrorKind::PermissionDenied,
        format!(
            "failed to replace app bundle at {} without administrator escalation: {err}",
            path.display()
        ),
    ))
}
...
#[cfg(target_os = "macos")]
fn move_current_app_to_backup(current_app_path: &Path, backup_path: &Path) -> Result<()> {
    std::fs::rename(current_app_path, backup_path).map_err(|err| {
        if err.kind() == std::io::ErrorKind::PermissionDenied {
            macos_direct_replace_permission_denied(current_app_path, err)
        } else {
            err.into()
        }
    })
...
impl Update {
    ...
    fn install_inner(&self, bytes: &[u8]) -> Result<()> {
        ...
        if let Err(err) = move_current_app_to_backup(
            &self.extract_path,
...
mod tests {
    ...
    #[test]
    #[cfg(target_os = "macos")]
    fn macos_direct_replace_permission_denied_stays_permission_denied() {
        use crate::error::Error;
        use std::path::Path;

        let err = macos_direct_replace_permission_denied(
        ...
        match err {
            Error::Io(io_err) => {
                ...
                assert!(message.contains("/Applications/Test.app"));
                assert!(message.contains("without administrator escalation"));
            }
    ...
    #[test]
    #[cfg(target_os = "macos")]
    fn macos_move_current_app_to_backup_uses_direct_rename() {
        let root = tempfile::tempdir().expect("temp dir");
        let current = root.path().join("Test.app");
        let backup = root.path().join("backup").join("current_app");
        std::fs::create_dir_all(current.join("Contents")).expect("create current app");
        std::fs::create_dir_all(backup.parent().expect("backup parent"))
            .expect("create backup dir");

        move_current_app_to_backup(&current, &backup).expect("direct rename should succeed");

        assert!(!current.exists(), "current app should be moved away");
        assert!(
            backup.exists(),
