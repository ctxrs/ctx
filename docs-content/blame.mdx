...
Blame is included in the open-source ctx CLI. [Install ctx](/getting-started/install)
or build it from [public source](https://github.com/ctxrs/ctx).
...
Blame reads retained Core history and a plain, rebuildable attribution index;
it needs no account, activation, or separate encrypted store. Existing legacy
data and keys are left untouched and are not used by ctx 1.5.
