...
This policy explains how ctx engineering inc, a Delaware corporation, handles information for ctx. It covers the public documentation site, support channels, and the ctx local search CLI. The official website is [ctx.rs](https://ctx.rs), the official X account is [@ctx_rs](https://x.com/ctx_rs), and the official source repository is [github.com/ctxrs/ctx](https://github.com/ctxrs/ctx).
...
ctx 1.5 does not start trials, evaluate subscriptions, collect machine
fingerprints, or access legacy product keys. Blame uses local retained Core
history and a plain derived attribution index. Existing legacy files and keys
are left untouched. Existing hosted account, trial, billing, and referral
records are covered in [Legacy services and support](/legal/legacy-services);
upgrading the CLI does not delete those records or cancel a subscription.
