# Docs

This directory contains selected ctx documentation for GitHub browsing.

## Start here

- [Overview](index.mdx)
- [Install and Launch](getting-started/install-and-launch.mdx)
- [Work Records](work-records.mdx)
- [Workbench Tour](workbench/tour.mdx)
- [Work Model](agent-work-model.mdx)
- [Work and ADE Extension Decisions](work-ade-extension-decisions.mdx)
- [Work Namespace Compatibility](work-namespace-compatibility.mdx)
- [Work Source Of Truth Contract](work-source-of-truth-contract.mdx)
- [ACP Provider Plugin Contract](acp-provider-plugin-contract.mdx)
- [ACP v1 Conformance Target](acp-v1-conformance-target.mdx)
- [Plugin Contribution Contract](plugin-contribution-contract.mdx)
- [What Is a Worktree?](what-is-a-worktree.mdx)
- [Containerization](containerization.mdx)

## Sections

- `getting-started/`: onboarding and first-run flows
- `workbench/`: optional desktop GUI surfaces over local Work records
- `settings/`: provider, safety, and update configuration
- `worktrees-runtime/`: worktree concepts and runtime behavior
- `examples/`: design examples and local implementation boundary sketches
- `troubleshooting/`: help and support content
- `legal/`: privacy, security, and terms pages
- `blog/`: selected long-form writing

## Examples

- [Harness Starter Boundaries](examples/harness-starter-boundaries.md)
- [Example Work Report](examples/work-report.md)
- [Work Observability E2E Walkthrough](examples/work-observability-e2e.md)

Full rendered docs: https://ctx.rs
