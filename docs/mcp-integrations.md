...
The installer parses JSON, TOML, and YAML configs with structured parsers,
...
| MiMo Code | Yes, implemented | `~/.config/mimocode/mimocode.jsonc`, or `MIMOCODE_HOME/config/mimocode.jsonc` | `.mimocode/mimocode.jsonc` | JSON/JSONC-style config with `mcp.ctx`, local server uses `type: "local"` and command argv array | Parse JSON and upsert `mcp.ctx`; remove by deleting that key | `MIMOCODE_HOME` or `~/.config/mimocode` | [MiMo Code README](https://github.com/XiaomiMiMo/MiMo-Code), [MiMo MCP config](https://github.com/XiaomiMiMo/MiMo-Code/blob/main/packages/opencode/src/config/mcp.ts), [ctx providers](provider-support.md) | ctx writes strict JSON into `.jsonc`; MiMo accepts JSONC but comments in an existing file are reported as invalid. |
