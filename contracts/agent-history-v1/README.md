...
The required status/init property `localOnly` is deprecated but retained in
1.3.2, including the existing SDK accessors and constructor arguments. It is a
legacy compatibility marker, not a claim that every configured operation is
offline or that an external semantic executor keeps content on this machine.
Inspect semantic executor diagnostics and the documented network settings for
those decisions. Nested semantic fields and opaque event payloads retain their
own meaning; they must not be rewritten to match this marker.

CLI/MCP status and setup retain schema version 2. Generic usage/error, index,
and daemon command envelopes retain schema version 1 and `local_only`. Removing
these fields requires an explicitly versioned replacement contract, not a
silent change to `agent-history-v1`.
