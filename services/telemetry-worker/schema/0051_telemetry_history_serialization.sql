...
select pg_advisory_xact_lock(
  hashtext('ctx.materialize_product_telemetry_history.v3')
);
...
begin
  if current_user <> 'ctx_migration'
    ...
    or not exists (
      ...
      where oid = to_regprocedure('ctx.materialize_product_telemetry_history(date)')
        ...
        and md5(prosrc) in ('3f90248e9f76a1fab881b1f2acca6711', '7da1e052fd9bdf62376ed25bb3870743')
    )
  ...
  then
    raise exception
      'telemetry history serialization requires ctx_migration, the reviewed materializer, and valid receipt indexes';
  end if;
end
$precondition$;
...
begin
  ...
  -- The transaction-wide lock also covers old one-day callers and multi-day
  -- backfills. Per-day locks could deadlock callers using opposite date orders.
  perform pg_advisory_xact_lock(
    hashtext('ctx.telemetry_diagnostic_daily'),
    hashtext('materialize_product_telemetry_history')
  );
...
comment on function ctx.materialize_product_telemetry_history(date) is
  'Serializes overlapping materialization transactions and idempotently replaces one complete UTC receipt day of permanent identity-free diagnostics; it never deletes raw rows.';
