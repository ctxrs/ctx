...
fn windows_sanitized_process_supervisor_script_with_provenance(
    ...
    // A recycled child PID can still name the previous durable lock record.
    // Unknown read failures retry before spawning; absent or malformed metadata has no old ID.
    Ok(format!(
        "$ErrorActionPreference='Stop';{process}$lockPath='{daemon_lock}';$ownerPath='{owner_provenance}';$tempPath=$ownerPath+'.'+$PID+'.tmp';Remove-Item -LiteralPath $ownerPath -Force -ErrorAction SilentlyContinue;Remove-Item -LiteralPath $tempPath -Force -ErrorAction SilentlyContinue;[int]$delay=2;while($true){{$c=$null;$code=1;$started=[DateTime]::UtcNow;try{{$previousOwner=$null;$previousText=$null;try{{$previousText=Get-Content -LiteralPath $lockPath -Raw -ErrorAction Stop}}catch [System.Management.Automation.ItemNotFoundException]{{}};try{{$previousLock=$previousText|ConvertFrom-Json -ErrorAction Stop;$previousOwner=[string]$previousLock.owner_id}}catch{{}};$c=[Diagnostics.Process]::Start($p);$owner=$null;while(!$c.HasExited -and $null -eq $owner){{try{{$lock=Get-Content -LiteralPath $lockPath -Raw -ErrorAction Stop|ConvertFrom-Json -ErrorAction Stop;if(([uint32]$lock.pid -eq [uint32]$c.Id)-and(-not [string]::IsNullOrWhiteSpace([string]$lock.owner_id))-and([string]$lock.owner_id -cne $previousOwner)){{$owner=[string]$lock.owner_id}}}}catch{{}};if($null -eq $owner){{Start-Sleep -Milliseconds 25}}}};if($null -ne $owner){{$record=[ordered]@{{schema_version=1;pid=[uint32]$c.Id;owner_id=$owner}}|ConvertTo-Json -Compress;[IO.File]::WriteAllText($tempPath,$record,(New-Object Text.UTF8Encoding($false)));Move-Item -LiteralPath $tempPath -Destination $ownerPath -Force}};$c.WaitForExit();$code=$c.ExitCode}}catch{{$code=1}}finally{{Remove-Item -LiteralPath $ownerPath -Force -ErrorAction SilentlyContinue;Remove-Item -LiteralPath $tempPath -Force -ErrorAction SilentlyContinue;if($null -ne $c){{$c.Dispose()}}}};if($code -eq 0){{exit 0}};if(([DateTime]::UtcNow-$started).TotalSeconds -ge 60){{$delay=2}};Start-Sleep -Seconds $delay;$delay=[Math]::Min($delay*2,60)}}"
    ))
}
