...
impl ArtifactFetcher for TestArtifactFetcher {
    fn fetch_to_writer(
        ...
        let path = request
            .endpoint()
            .strip_prefix("file://")
            .ok_or_else(|| anyhow!("test fetcher requires a file URL"))?;
        let bytes = fs::read(path)?;
        if bytes.len() as u64 > request.max_bytes() {
            return Err(anyhow!("test artifact exceeds max bytes"));
        }
...
const TEST_ARTIFACT_FETCHER: TestArtifactFetcher = TestArtifactFetcher;

#[derive(Default)]
struct CountingArtifactFetcher {
    calls: std::sync::atomic::AtomicUsize,
}

impl ArtifactFetcher for CountingArtifactFetcher {
    fn fetch_to_writer(
        &self,
        request: ArtifactFetchRequest<'_>,
        writer: &mut dyn std::io::Write,
        ...
        TEST_ARTIFACT_FETCHER.fetch_to_writer(request, writer)
    }
}

impl CountingArtifactFetcher {
    fn calls(&self) -> usize {
...
fn signed_archive_hash_mismatch_fails_closed_without_publication() {
    ...
    let descriptor = test_descriptor(&artifact_url, A_HASH, B_HASH);
    let error =
        acquire_coreml_bundle_for(temp.path(), &descriptor, &TEST_ARTIFACT_FETCHER).unwrap_err();
...
fn daemon_signed_acquisition_installs_clean_cache_then_uses_cache_only() {
    ...
    let fetcher = CountingArtifactFetcher::default();

    let acquired = acquire_coreml_bundle_for(&cache, &descriptor, &fetcher).unwrap();
    assert_eq!(acquired.source, CoreMlAcquisitionSource::Download);
    assert_eq!(fetcher.calls(), 1, "daemon cache miss fetches exactly once");
    ...
    let cached = acquire_coreml_bundle_for(&cache, &descriptor, &fetcher).unwrap();
    assert_eq!(cached.source, CoreMlAcquisitionSource::Cache);
    assert_eq!(cached.bundle.manifest_sha256, manifest_sha256);
    assert_eq!(
        fetcher.calls(),
        1,
        "daemon cache hit must not invoke the fetch capability"
    );
}
...
fn acquisition_lock_keeps_repair_away_from_active_publication() {
    ...
    std::thread::scope(|scope| {
        let second = scope.spawn(|| {
            ...
            acquire_coreml_bundle_for(&cache, &descriptor, &TEST_ARTIFACT_FETCHER)
        });
...
fn daemon_acquisition_repairs_bundle_published_without_marker() {
    ...
    let descriptor = test_descriptor(&artifact_url, &archive_sha256, &manifest_sha256);
    let cache = temp.path().join("cache");
    acquire_coreml_bundle_for(&cache, &descriptor, &TEST_ARTIFACT_FETCHER).unwrap();
    ...
    let repaired = acquire_coreml_bundle_for(&cache, &descriptor, &TEST_ARTIFACT_FETCHER).unwrap();
...
fn daemon_acquisition_refuses_to_repair_symlinked_incomplete_entries() {
    ...
    let error = acquire_coreml_bundle_for(&directory_cache, &descriptor, &TEST_ARTIFACT_FETCHER)
        .unwrap_err();
    assert!(model_acquisition_integrity_error(&error));
    ...
    let error =
        acquire_coreml_bundle_for(&marker_cache, &descriptor, &TEST_ARTIFACT_FETCHER).unwrap_err();
    assert!(model_acquisition_integrity_error(&error));
...
fn daemon_acquisition_refuses_to_repair_through_symlinked_parent() {
    ...
    let error = acquire_coreml_bundle_for(&cache, &descriptor, &TEST_ARTIFACT_FETCHER).unwrap_err();
    assert!(model_acquisition_integrity_error(&error));
...
fn daemon_acquisition_does_not_repair_completed_integrity_failures() {
    ...
    let marker_cache = temp.path().join("marker-cache");
    acquire_coreml_bundle_for(&marker_cache, &descriptor, &TEST_ARTIFACT_FETCHER).unwrap();
    let marker_bundle =
        content_addressed_bundle_path(&marker_cache, descriptor.manifest_sha256).unwrap();
    ...
    acquire_coreml_bundle_for(&content_cache, &descriptor, &TEST_ARTIFACT_FETCHER).unwrap();
    let content_bundle =
    ...
    let error =
        acquire_coreml_bundle_for(&content_cache, &descriptor, &TEST_ARTIFACT_FETCHER).unwrap_err();
    assert!(model_acquisition_integrity_error(&error));
