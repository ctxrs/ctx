...
pub(crate) fn fixed_time() -> chrono::DateTime<Utc> {
...
pub(crate) fn excluded_filter(session_id: Option<Uuid>) -> SearchFilters {
    SearchFilters {
...
pub(crate) fn test_store() -> (tempfile::TempDir, ctx_history_store::Store) {
    let temp = tempdir();
...
pub(crate) fn maybe_write_synthetic_search_smoke_artifact() {
