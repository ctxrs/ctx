...
impl ProStorageEvidence {
    pub fn validate(&self) -> Result<(), ProtocolError> {
        // Graph manifest V3 remains admissible as authenticated compatibility
        // evidence; V4 is current publication evidence.
        // Materializer checkpoints V4 through V7 remain admissible only as
        // authenticated status evidence while a helper control-loads a prior
        // active graph for migration. Checkpoint V8 is current publication
        // evidence; accepting a version here grants no EventIndex read
        // compatibility between generations.
        if !matches!(self.graph_manifest_schema, 3 | 4)
            || self.flat_format_version != 2
            || !matches!(self.materializer_checkpoint_version, 4 | 5 | 6 | 7 | 8)
