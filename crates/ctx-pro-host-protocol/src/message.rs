...
impl ProStorageEvidence {
    pub fn validate(&self) -> Result<(), ProtocolError> {
        // V4 through V6 remain admissible only as authenticated status
        // evidence while a helper control-loads a prior active graph for
        // migration. Accepting a version here grants no EventIndex read
        // compatibility between those storage generations.
        if self.graph_manifest_schema != 3
            || self.flat_format_version != 2
            || !matches!(self.materializer_checkpoint_version, 4 | 5 | 6 | 7)
