...
impl ProStorageEvidence {
    pub fn validate(&self) -> Result<(), ProtocolError> {
        // V4 through V7 remain admissible only as authenticated status
        // evidence while a helper control-loads a prior active graph for
        // migration. V8 is current publication evidence; accepting a version
        // here grants no EventIndex read compatibility between generations.
