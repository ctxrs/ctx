...
use crate::model::{
    GitObjectFormat, GitObjectId, OUTCOME_CAPTURE_REVISION, RepositoryAbstentionReason,
    RepositoryAlias, RepositoryAliasKind, RepositoryCandidateKind, RepositoryEvidenceKind,
...
use super::{
    LinkedOutcomeInput, LiteralFileObservation, LiteralRepositoryFileInvocation,
    LiteralVcsObservation, NeutralRepositoryFacts, RepositoryEvidenceResolver, evaluate,
...
fn has_reason(
    annotation: &crate::model::RepositoryEvaluation,
...
fn multi_repository_candidate_evidence_is_complete_and_input_order_independent() {
    ...
    let forward = evaluate(NeutralRepositoryFacts {
        file_observations: observations.to_vec(),
        ..NeutralRepositoryFacts::default()
    });
    let reverse = evaluate(NeutralRepositoryFacts {
...
fn exact_file_invocations_scope_and_canonicalize_without_observation_promotion() {
    ...
    let read = LiteralRepositoryFileInvocation {
        operation_ordinal: 2,
    ...
    let write = LiteralRepositoryFileInvocation {
        operation_ordinal: 1,
    ...
    let rename = LiteralRepositoryFileInvocation {
        operation_ordinal: 0,
...
fn invocation_intent_never_asserts_a_file_effect_observation() {
    ...
    let annotation = evaluate(NeutralRepositoryFacts {
        declared_tool_workdir: Some(repo.to_string_lossy().into_owned()),
        repository_file_invocation_evidence: vec![LiteralRepositoryFileInvocation {
            operation_ordinal: 0,
            path: "src/new.rs".to_owned(),
            prior_path: None,
...
fn duplicate_invocation_and_observation_paths_share_one_candidate_slot() {
    ...
    for index in 0..32 {
        ...
        file_observations.push(LiteralFileObservation {
...
#[test]
fn strict_candidate_overflow_preserves_admissible_ordinary_evaluation() {
    let temp = TempDir::new().unwrap();
...
fn mixed_candidate_channels_are_admitted_at_the_shared_ceiling() {
    ...
    for index in 0..10 {
        ...
        fs::create_dir(&path).unwrap();
        vcs_observations.push(LiteralVcsObservation {
...
fn one_session_can_bind_two_repositories_without_crossing_files() {
    ...
    let first_event = resolver.evaluate(NeutralRepositoryFacts {
        session_cwd: Some(control.to_string_lossy().into_owned()),
    ...
    let second_event = resolver.evaluate(NeutralRepositoryFacts {
        session_cwd: Some(control.to_string_lossy().into_owned()),
...
fn relative_absolute_cd_and_repeated_git_c_are_literal_and_multi_repo() {
    ...
    let relative = evaluate(NeutralRepositoryFacts {
        session_cwd: Some(control.to_string_lossy().into_owned()),
...
fn literal_cd_is_preserved_as_candidate_evidence_but_not_opaque_operation_authority() {
    ...
    let cargo = evaluate(NeutralRepositoryFacts {
        session_cwd: Some(control.to_string_lossy().into_owned()),
...
fn opaque_command_suppresses_only_command_and_session_guesses() {
    ...
    let annotation = evaluate(NeutralRepositoryFacts {
        ...
        vcs_observations: vec![LiteralVcsObservation {
...
fn certification_does_not_synthesize_vcs_activity_but_explicit_activity_survives() {
    let temp = TempDir::new().unwrap();
    let repo = repository(temp.path(), "repo", None);
    let certified = evaluate(NeutralRepositoryFacts {
        declared_tool_workdir: Some(repo.to_string_lossy().into_owned()),
        ..NeutralRepositoryFacts::default()
    });
    ...
    let explicit = evaluate(NeutralRepositoryFacts {
        declared_tool_workdir: Some(repo.to_string_lossy().into_owned()),
        vcs_observations: vec![LiteralVcsObservation {
...
fn origin_authority_survives_secondary_remote_addition_and_config_reordering() {
    ...
    let before = evaluate(NeutralRepositoryFacts {
        declared_tool_workdir: Some(repo.to_string_lossy().into_owned()),
        ..NeutralRepositoryFacts::default()
    });
    run_git(
    ...
    let added = evaluate(NeutralRepositoryFacts {
        declared_tool_workdir: Some(repo.to_string_lossy().into_owned()),
        ..NeutralRepositoryFacts::default()
    });
    ...
    let reordered = evaluate(NeutralRepositoryFacts {
        declared_tool_workdir: Some(repo.to_string_lossy().into_owned()),
        ..NeutralRepositoryFacts::default()
    });
...
fn move_preserves_certified_identity_but_old_path_first_seen_missing_abstains() {
    ...
    let mut resolver = RepositoryEvidenceResolver::default();
    let first = resolver.evaluate(NeutralRepositoryFacts {
        declared_tool_workdir: Some(old.to_string_lossy().into_owned()),
        ..NeutralRepositoryFacts::default()
    });
    let new = temp.path().join("new");
    fs::rename(&old, &new).unwrap();
    let moved = resolver.evaluate(NeutralRepositoryFacts {
    ...
    let missing = resolver.evaluate(NeutralRepositoryFacts {
        declared_tool_workdir: Some(old.to_string_lossy().into_owned()),
        ..NeutralRepositoryFacts::default()
    });
...
fn provider_native_identity_survives_missing_or_moved_local_path_without_authorization() {
    ...
    assert!(
        certified.repository_bindings[0]
    ...
    let retained = evaluate(NeutralRepositoryFacts {
        provider_native_repository_aliases: vec![alias.clone()],
        declared_tool_workdir: Some(old.to_string_lossy().into_owned()),
        ..NeutralRepositoryFacts::default()
    });
    ...
    assert!(
        retained.repository_bindings[0]
            .local_root_authorization
            .is_none()
    );
    assert!(!has_reason(
    ...
    let never_present = evaluate(NeutralRepositoryFacts {
    ...
    assert!(
        never_present.repository_bindings[0]
            .local_root_authorization
            .is_none()
    );
}
...
fn conflicting_or_unbounded_provider_native_identities_abstain() {
    let conflict = evaluate(NeutralRepositoryFacts {
    ...
    let ambiguous = evaluate(NeutralRepositoryFacts {
    ...
    assert!(
        independent.repository_bindings[0]
            .local_root_authorization
            .is_some()
    );
    assert!(has_reason(
    ...
    let no_session_fallback = evaluate(NeutralRepositoryFacts {
        provider_native_repository_aliases: vec![forge("acme", "one"), forge("acme", "two")],
...
fn provider_prebounded_oversized_command_blocks_session_cwd_fallback() {
    ...
    let annotation = evaluate(NeutralRepositoryFacts {
        session_cwd: Some(repository.to_string_lossy().into_owned()),
        command_disposition: super::CommandLiteralDisposition::CommandTooLarge,
        ..NeutralRepositoryFacts::default()
    });
