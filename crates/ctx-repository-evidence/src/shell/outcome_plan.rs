...
use crate::model::{RepositoryAbstentionReason, RepositoryCommitOperationKind};

use super::{
    MAX_COMMAND_BYTES, known_git_builtin, lexical_absolute, literal_cd_destination,
    strip_comments_and_bound_heredocs, tokenize, unwrap_command_wrappers,
};

#[derive(Debug, Clone, Copy, PartialEq, Eq, serde::Serialize)]
pub enum BoundedCommitProducer {
    Commit,
    Merge,
    Rebase,
    CherryPick,
...
pub enum BoundedOutcomeOperation {
    Commit {
...
pub struct BoundedOutcomePlan {
    pub operation: BoundedOutcomeOperation,
    pub operation_source_oid: Option<String>,
    pub operation_repository_path: PathBuf,
    pub output_repository_path: Option<PathBuf>,
    pub machine_output_isolated: bool,
    pub expected_pr_repository_path: Option<Vec<String>>,
    pub expected_pr_number: Option<u64>,
}

#[derive(Debug, Clone, PartialEq, Eq)]
pub enum BoundedOutcomePlanDisposition {
    Unrecognized,
...
pub fn bounded_outcome_plan(command: &str, base: &Path) -> BoundedOutcomePlanDisposition {
    ...
    for (segment_index, segment) in tokenization.segments.into_iter().enumerate() {
        ...
        let (unwrapped, wrapper_error) = unwrap_outcome_command(&segment);
...
fn unwrap_outcome_command(segment: &[String]) -> (Option<&[String]>, Option<&'static str>) {
    if segment.first().map(String::as_str) != Some("ctx-build-governor") {
        return unwrap_command_wrappers(segment);
    }
    if segment.get(1).map(String::as_str) != Some("exec")
        || segment.get(2).map(String::as_str) != Some("--")
        || segment.len() == 3
    {
        return (None, Some("unsupported_ctx_build_governor_wrapper"));
    }
    if segment[3..]
        .iter()
        .any(|argument| argument.contains('$') || argument.contains('`'))
    {
        return (None, Some("dynamic_ctx_build_governor_command"));
    }
    unwrap_command_wrappers(&segment[3..])
}
...
fn bounded_gh_operation(
    ...
    while let Some(argument) = arguments.get(index) {
        ...
        if let Some(value) = value {
            ...
            && let Ok(number) = argument.parse::<u64>()
            && (number == 0 || expected_pr_number.replace(number).is_some())
        {
            return None;
        }
        index += 1;
