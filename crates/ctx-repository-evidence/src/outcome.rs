...
use crate::model::{
    GitObjectFormat, GitObjectId, MAX_COMMIT_OPERATION_MAPPINGS, OUTCOME_CAPTURE_REVISION,
    RepositoryAbstentionReason, RepositoryAlias, RepositoryCommitMapping,
    RepositoryCommitOperationEvent, RepositoryCommitOperationKind, RepositoryCommitOperationState,
...
use super::{
    ...
    LiteralPullRequestAssociationObservation, bounded_outcome_plan, exact_pull_request_association,
    lexical_absolute,
};
...
pub enum LiteralOutcomeObservation {
...
impl From<RepositoryOutcomeObservation> for LiteralOutcomeObservation {
...
#[derive(Debug, Clone, PartialEq, Eq, serde::Serialize)]
pub struct DeferredCommitObservation {
    pub oid_prefix: String,
    pub subject: String,
    pub producer: BoundedCommitProducer,
    pub observed_at_unix_ms: i64,
    pub linkage: RepositoryOutcomeLinkage,
}

#[derive(Debug, Clone, PartialEq, Eq, serde::Serialize)]
pub struct DeferredCommitOperationObservation {
...
#[derive(Debug, Clone, PartialEq, Eq, serde::Serialize)]
pub struct DeferredCherryPickObservation {
    pub source: GitObjectId,
    pub result_oid_prefix: String,
    pub result_subject: String,
    pub observed_at_unix_ms: i64,
    pub linkage: RepositoryOutcomeLinkage,
}

pub struct LinkedOutcomeInput<'a> {
    pub provider: &'static str,
    pub command: &'a str,
    pub session_cwd: Option<&'a str>,
    pub declared_workdir: Option<&'a str>,
    pub origin_call_id: &'a str,
    pub result_call_id: &'a str,
    pub origin_event_sequence: u64,
    pub continuation_call_id_sha256: &'a [[u8; 32]],
    pub result_record_sha256: [u8; 32],
    pub observed_at_unix_ms: i64,
    pub result_succeeded: bool,
    pub result_output: &'a Value,
    ...
    pub structured_commit_oid: Option<&'a str>,
    pub output_repository_path: Option<&'a str>,
}
...
pub struct LinkedOutcomeEvidence {
    ...
    pub outcomes: Vec<LiteralOutcomeObservation>,
    pub pull_request_associations: Vec<LiteralPullRequestAssociationObservation>,
    pub abstentions: Vec<(RepositoryAbstentionReason, &'static str)>,
}

pub fn linked_outcome_evidence(input: LinkedOutcomeInput<'_>) -> Option<LinkedOutcomeEvidence> {
    ...
    if let Some(base) = declared_base.as_deref().filter(|base| {
        ...
        return Some(LinkedOutcomeEvidence {
            provider_native_repository_aliases: vec![
                association.pull_request.forge_repository.clone(),
            ],
    ...
    match parsed {
        OperationResult::Exact { outcome, aliases } => {
            ...
            Some(LinkedOutcomeEvidence {
                ...
                outcomes: vec![LiteralOutcomeObservation::Exact(outcome)],
        ...
        OperationResult::Deferred(deferred) => Some(LinkedOutcomeEvidence {
            ...
            outcome_operation_repository_path: operation_path,
            outcome_output_repository_path: output_path,
            outcomes: vec![LiteralOutcomeObservation::DeferredCommit(deferred)],
            pull_request_associations: Vec::new(),
            abstentions: Vec::new(),
        }),
        OperationResult::DeferredOperation(deferred) => Some(LinkedOutcomeEvidence {
            ...
            outcome_operation_repository_path: operation_path,
            outcome_output_repository_path: output_path,
            outcomes: vec![LiteralOutcomeObservation::DeferredCommitOperation(deferred)],
            pull_request_associations: Vec::new(),
            abstentions: Vec::new(),
        }),
        OperationResult::DeferredCherryPick(deferred) => Some(LinkedOutcomeEvidence {
            ...
            outcome_operation_repository_path: operation_path,
            outcome_output_repository_path: output_path,
            outcomes: vec![LiteralOutcomeObservation::DeferredCherryPick(deferred)],
            pull_request_associations: Vec::new(),
            abstentions: Vec::new(),
        }),
...
fn parse_operation_result(
    ...
    match plan.operation {
        BoundedOutcomeOperation::Commit {
            ...
            mappings.sort();
            if mappings.len() > MAX_COMMIT_OPERATION_MAPPINGS
...
fn exact_commit_result(
    ...
    if exact_oid_output
        && let Some(object_id) = exact_machine_oid_output(output, producer, machine_output_isolated)
    {
...
fn exact_replacements(value: &Value) -> Option<Vec<RepositoryCommitMapping>> {
    ...
    if values.is_empty() || values.len() > MAX_COMMIT_OPERATION_MAPPINGS {
        return None;
    }
