...
pub use outcome_plan::{
    ...
    BoundedOutcomePlanDisposition, bounded_outcome_plan,
};
pub use pull_request::bounded_pull_request_association_query;

use crate::model::{RepositoryAbstentionReason, RepositoryEvidenceKind};
...
pub fn lexical_absolute(value: &str, base: Option<&Path>) -> Option<PathBuf> {
    if value.is_empty()
        || value == "-"
        || value.starts_with('~')
...
pub(super) fn analyze(command: Option<&str>, base: Option<&Path>) -> CommandAnalysis {
    ...
    if !cut_off && let Some((reason, detail)) = tokenization.terminal_abstention {
        analysis.blocks_session_fallback = true;
        push_analysis_abstention(&mut analysis, reason, detail);
    }
...
struct Tokenization {
    ...
    unconditional_separators_after_segments: Vec<usize>,
...
fn tokenize(command: &str) -> Result<Tokenization, (RepositoryAbstentionReason, &'static str)> {
    ...
    let mut unconditional_separators_after_segments = Vec::new();
    ...
    while let Some(character) = characters.next() {
        if escaped {
            match quote {
                Some('\0') => token.push(match character {
                    'n' => '\n',
                    'r' => '\r',
                    't' => '\t',
                    '\\' | '\'' | '"' => character,
        ...
        // Expansion must be rejected before quote removal can turn an unknown
        // option into an apparently literal producer argument.
        let dynamic = if quote == Some('"') {
            matches!(character, '$' | '`')
        } else {
            quote.is_none()
                && matches!(character, '$' | '`' | '*' | '?' | '[' | ']' | '{' | '}')
        ...
        if dynamic {
            return Ok(Tokenization {
                ...
                segments,
                unconditional_separators_after_segments,
        ...
        match character {
            ...
            '\n' => {
                ...
                unconditional_separators_after_segments.push(segments.len() - 1);
