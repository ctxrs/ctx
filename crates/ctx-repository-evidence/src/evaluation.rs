...
use super::{
    BoundedCommitProducer, BoundedOutcomePlanDisposition, CommandLiteralDisposition,
    ...
    identity::{
        ProviderIdentityResolution, binding_accepts_forge_repository, push_abstention,
        reconcile_provider_identity, resolve_provider_native_identity,
    },
    outcome::LiteralOutcomeObservation,
...
use crate::model::{
    ...
    RepositoryAbstention, RepositoryAbstentionReason, RepositoryCandidateKind,
    RepositoryCommitMapping, RepositoryCommitOperationKind, RepositoryEvaluation,
...
pub const MAX_REPOSITORY_CANDIDATES: usize = 32;
...
pub(super) struct ScopedVcsInput {
    ...
    pub(super) observation: LiteralVcsObservation,
}

pub fn evaluate(input: NeutralRepositoryFacts) -> RepositoryEvaluation {
    RepositoryEvidenceResolver::default().evaluate(input)
}

pub(super) fn evaluate_with_resolver(
    input: NeutralRepositoryFacts,
    resolver: &mut RepositoryEvidenceResolver,
) -> RepositoryEvaluation {
    let activity_at_unix_ms = input
        .activity_at_unix_ms
        .unwrap_or(MISSING_ACTIVITY_TIME_UNIX_MS);
    ...
    let mut annotation = RepositoryEvaluation::default();
    ...
    let command_analysis = match input.command_disposition {
        CommandLiteralDisposition::Analyze => analyze(input.command.as_deref(), base),
        CommandLiteralDisposition::CommandTooLarge => command_too_large(),
    };
    annotation
    ...
    let has_bounded_outcome_route =
        input
            ...
            && matches!(
                ...
                CommandLiteralDisposition::Analyze
            );
    ...
    if (!command_analysis.blocks_session_fallback || has_bounded_outcome_route)
        && let Some(path) = &outcome_operation_path
    {
        candidates.push(Candidate {
            path: path.clone(),
            kind: CandidateKind::Directory,
    ...
    for candidate in candidates {
        match resolver.certify(&candidate, activity_at_unix_ms, &mut probe_budget) {
...
fn resolve_deferred_commit_observations(
    ...
    observations: Vec<LiteralOutcomeObservation>,
    ...
    for observation in observations {
        let deferred = match observation {
            LiteralOutcomeObservation::Exact(outcome) => {
                exact.push(*outcome);
                continue;
            }
            LiteralOutcomeObservation::DeferredCommitOperation(deferred) => {
                if deferred.mappings.is_empty()
                    || deferred.mappings.len() > MAX_COMMIT_OPERATION_MAPPINGS
                {
                    push_abstention(
                        annotation,
                        RepositoryEvidenceKind::ProviderNativeResult,
                ...
                let Ok(commit_operation) =
                    crate::model::RepositoryCommitOperationEvent::repository_verified_yield(
                        &deferred.linkage,
                        deferred.kind,
                        deferred.mappings,
            ...
            LiteralOutcomeObservation::DeferredCherryPick(deferred) => {
                let Some(operation_path) = operation_path else {
                    push_abstention(
                        annotation,
                        RepositoryEvidenceKind::ProviderNativeResult,
            ...
            LiteralOutcomeObservation::DeferredCommit(deferred) => deferred,
        };
        let Some(operation_path) = operation_path else {
            push_abstention(
                annotation,
...
mod associations;
use associations::enrich_pull_request_associations;
fn bounded_absolute(
    value: Option<&str>,
