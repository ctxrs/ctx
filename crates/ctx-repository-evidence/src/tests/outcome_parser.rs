use crate::model::{
    MAX_COMMIT_OPERATION_MAPPINGS, RepositoryAbstentionReason, RepositoryCommitOperationKind,
    RepositoryOutcomeKind, RepositoryOutcomeObservation,
};
use serde_json::Value;

use super::super::{
    BoundedCommitProducer, LinkedOutcomeInput, LiteralOutcomeObservation, linked_outcome_evidence,
};

fn exact_outcome(value: &LiteralOutcomeObservation) -> &RepositoryOutcomeObservation {
    match value {
        LiteralOutcomeObservation::Exact(outcome) => outcome,
        LiteralOutcomeObservation::DeferredCommit(_)
        | LiteralOutcomeObservation::DeferredCommitOperation(_)
        | LiteralOutcomeObservation::DeferredCherryPick(_) => {
    ...
    for (command, output, expected_prefix, expected_subject) in [
        (
            ...
            serde_json::json!([
                ...
                {"type": "input_text", "text": "[main 5555555] Fail closed on invalid retry headers\n 2 files changed, 24 insertions(+), 5 deletions(-)\n"},
        ...
        let LiteralOutcomeObservation::DeferredCommit(deferred) = &evidence.outcomes[0] else {
...
fn exact_commit_receipt_survives_unrelated_output_after_exact_head() {
    ...
    let output = Value::String(format!(
        "[test/receipt-with-output 123456789] fix: reserve output bytes before reading input\n 2 files changed, 24 insertions(+), 5 deletions(-)\n{oid}\npub const MAX_PAGE_BYTES: usize = 64 * 1024 * 1024;\n"
    ));
    let command = concat!(
        "git commit -m 'fix: reserve output bytes before reading input' && ",
        "git status --short && git rev-parse HEAD && ",
        "sed -n '12,18p' crates/ctx-attribution-model/src/lib.rs"
    );
    let evidence = linked_outcome_evidence(input(command, &output)).unwrap();
    let LiteralOutcomeObservation::DeferredCommit(deferred) = &evidence.outcomes[0] else {
        panic!("expected certified-receipt candidate");
    };
    assert_eq!(deferred.oid_prefix, "123456789");
    assert_eq!(
        deferred.subject,
        "fix: reserve output bytes before reading input"
    );

    let ambiguous = Value::String(format!(
        "[main 123456789] fix: reserve output bytes before reading input\n{oid}\n[main 1111111] unrelated second commit\n"
    ));
...
fn canonical_merge_graph_head_is_deferred_and_ambiguous_summaries_abstain() {
    let output = Value::String(
        "Merge made by the 'ort' strategy.\n README.md | 11 +++++++++++\n*   3333333 Merge retry validation documentation\n|\\  \n| * 4444444 Document retry validation contract\n* | 5555555 Fail closed on invalid retry headers\n"
            .to_owned(),
    );
    ...
    assert_eq!(deferred.oid_prefix, "3333333");
...
fn exact_rewrite_and_pull_request_schemas_are_supported() {
    ...
    let LiteralOutcomeObservation::DeferredCommitOperation(amend) = &rewrite.outcomes[0] else {
    ...
    let LiteralOutcomeObservation::DeferredCommitOperation(rebase) = &rebase.outcomes[0] else {
...
fn plural_rebase_mapping_is_explicit_unambiguous_and_bounded() {
    ...
    let [LiteralOutcomeObservation::DeferredCommitOperation(operation)] =
    ...
    let [LiteralOutcomeObservation::DeferredCommitOperation(reordered)] =
    ...
    let over_bound = (0..=MAX_COMMIT_OPERATION_MAPPINGS)
        .map(|index| {
            serde_json::json!({
...
fn native_cherry_pick_stdout_is_deferred_but_failures_and_ambiguity_abstain() {
    ...
    let evidence = linked_outcome_evidence(input(&command, &output)).unwrap();
    let [LiteralOutcomeObservation::DeferredCherryPick(deferred)] = evidence.outcomes.as_slice()
    else {
