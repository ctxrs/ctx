...
fn local_root_authorization_fingerprint_ignores_mutable_git_state_and_path_renames() {
    ...
    let initial = evaluate(NeutralRepositoryFacts {
        declared_tool_workdir: Some(repo.to_string_lossy().into_owned()),
        ..NeutralRepositoryFacts::default()
    });
    assert_eq!(
        initial.repository_bindings[0]
            .local_root_authorization
            .as_ref()
            .expect("certified local root")
            .local_root_authorization_fingerprint_revision,
        crate::model::LOCAL_ROOT_AUTHORIZATION_FINGERPRINT_REVISION
    );
    ...
    let changed = evaluate(NeutralRepositoryFacts {
        declared_tool_workdir: Some(repo.to_string_lossy().into_owned()),
        ..NeutralRepositoryFacts::default()
    });
    ...
    let moved = evaluate(NeutralRepositoryFacts {
...
fn local_root_authorization_fingerprint_matches_the_cross_repository_wire_encoding() {
    ...
    digest.update(crate::model::LOCAL_ROOT_AUTHORIZATION_FINGERPRINT_DOMAIN);
...
fn local_root_authorization_fingerprint_changes_for_recreated_root_git_dir_and_linked_worktree() {
    ...
    let linked_binding = evaluate(NeutralRepositoryFacts {
    ...
    let recreated_root = evaluate(NeutralRepositoryFacts {
        declared_tool_workdir: Some(repo.to_string_lossy().into_owned()),
        ..NeutralRepositoryFacts::default()
    });
    ...
    let recreated_git_dir = evaluate(NeutralRepositoryFacts {
        declared_tool_workdir: Some(repo.to_string_lossy().into_owned()),
        ..NeutralRepositoryFacts::default()
    });
    assert_ne!(
...
fn linked_worktree_pointer_rebind_recertifies_local_authorization() {
    ...
    let before = resolver.evaluate(NeutralRepositoryFacts {
    ...
    let rebound = resolver.evaluate(NeutralRepositoryFacts {
        declared_tool_workdir: Some(first.to_string_lossy().into_owned()),
        ..NeutralRepositoryFacts::default()
    });
...
fn moved_linked_worktree_recertifies_new_root_without_stale_old_authorization() {
    ...
    let after = resolver.evaluate(NeutralRepositoryFacts {
    ...
    let reused = resolver.evaluate(NeutralRepositoryFacts {
        declared_tool_workdir: Some(moved.to_string_lossy().into_owned()),
        ..NeutralRepositoryFacts::default()
    });
    ...
    let stale = resolver.evaluate(NeutralRepositoryFacts {
        declared_tool_workdir: Some(old.to_string_lossy().into_owned()),
        ..NeutralRepositoryFacts::default()
    });
...
fn moved_local_root_reuses_only_exact_prior_event_time_certificates() {
    ...
    let old_call = resolver.evaluate(NeutralRepositoryFacts {
    ...
    let old_file = resolver.evaluate(NeutralRepositoryFacts {
    ...
    let new_call = resolver.evaluate(NeutralRepositoryFacts {
    ...
    let replayed_call = resolver.evaluate(NeutralRepositoryFacts {
        activity_at_unix_ms: Some(100),
        session_cwd: Some(neutral.to_string_lossy().into_owned()),
        declared_tool_workdir: Some(old.to_string_lossy().into_owned()),
        command: Some("git status --short".to_owned()),
    ...
    assert!(
        replayed_call.repository_bindings[0]
            .local_root_authorization
            .is_none()
    );
    assert_eq!(resolver.full_certification_probe_count(), 2);

    let replayed_file = resolver.evaluate(NeutralRepositoryFacts {
        activity_at_unix_ms: Some(110),
        session_cwd: Some(neutral.to_string_lossy().into_owned()),
        declared_tool_workdir: Some(old.to_string_lossy().into_owned()),
        file_observations: vec![LiteralFileObservation {
    ...
    for input in [
        ...
        let untrusted = resolver.evaluate(input);
    ...
    let never_seen_file = resolver.evaluate(NeutralRepositoryFacts {
        activity_at_unix_ms: Some(110),
        file_observations: vec![LiteralFileObservation {
...
fn event_time_history_fails_closed_for_identity_conflict_and_live_replacement() {
    ...
    let mut resolver = RepositoryEvidenceResolver::default();
    let original = resolver.evaluate(NeutralRepositoryFacts {
        activity_at_unix_ms: Some(100),
        declared_tool_workdir: Some(old.to_string_lossy().into_owned()),
        ..NeutralRepositoryFacts::default()
    });
    fs::rename(&old, &moved).unwrap();
    let relocated = resolver.evaluate(NeutralRepositoryFacts {
        activity_at_unix_ms: Some(200),
        declared_tool_workdir: Some(moved.to_string_lossy().into_owned()),
        ..NeutralRepositoryFacts::default()
    });
    ...
    let historical_conflict = resolver.evaluate(NeutralRepositoryFacts {
        activity_at_unix_ms: Some(100),
        declared_tool_workdir: Some(old.to_string_lossy().into_owned()),
        ..NeutralRepositoryFacts::default()
    });
    ...
    let current_replacement = resolver.evaluate(NeutralRepositoryFacts {
        activity_at_unix_ms: Some(300),
    ...
    let mut conflicting_resolver = RepositoryEvidenceResolver::default();
    let local = conflicting_resolver.evaluate(NeutralRepositoryFacts {
    ...
    let forge = conflicting_resolver.evaluate(NeutralRepositoryFacts {
    ...
    let unbound_old_phase = conflicting_resolver.evaluate(NeutralRepositoryFacts {
        activity_at_unix_ms: Some(400),
        declared_tool_workdir: Some(conflict_old.to_string_lossy().into_owned()),
        ..NeutralRepositoryFacts::default()
    });
