...
fn exact_plural_rebase_receipt_certifies_all_source_result_mappings() {
    ...
    assert_eq!(
        operation.kind,
        crate::model::RepositoryCommitOperationKind::Rebase
    );
    ...
    let crate::model::RepositoryCommitOperationProof::RepositoryVerifiedYield(proof) =
        &operation.proof
    else {
...
fn native_cherry_pick_receipt_admits_exact_derivation_mapping() {
    ...
    assert_eq!(
        operation.kind,
        crate::model::RepositoryCommitOperationKind::CherryPick
    );
...
fn native_cherry_pick_substituted_after_source_read_abstains() {
    ...
    resolver.certifier = GitCertifier::for_test(wrapper, Duration::from_secs(2));
    ...
    assert!(resolver.git_subprocess_count() <= super::git::MAX_GIT_SUBPROCESSES_PER_EVENT);
    assert_eq!(
