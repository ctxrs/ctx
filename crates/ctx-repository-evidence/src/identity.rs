...
use super::{git::CertifiedCandidate, outcome::LiteralOutcomeObservation};
use crate::model::{
    REPOSITORY_ASSOCIATION_POLICY_REVISION, RepositoryAbstention, RepositoryAbstentionReason,
    RepositoryAlias, RepositoryAliasKind, RepositoryBinding, RepositoryEvaluation,
...
impl ProviderIdentityResolution {
    ...
    pub(super) fn binds_all_outcomes(&self, outcomes: &[LiteralOutcomeObservation]) -> bool {
        let Self::Binding(binding) = self else {
            return false;
        };
        !outcomes.is_empty()
            && outcomes.iter().all(|outcome| {
                let LiteralOutcomeObservation::Exact(outcome) = outcome else {
                    return false;
                };
