...
pub struct LiteralPullRequestAssociationObservation {
...
pub fn exact_pull_request_association(
    command: &str,
    declared_workdir: &str,
    output: &Value,
    linkage: RepositoryOutcomeLinkage,
) -> Option<LiteralPullRequestAssociationObservation> {
    ...
    Some(LiteralPullRequestAssociationObservation {
...
mod tests {
    ...
    fn bounded_literal_multiline_tail_is_accepted_but_never_supplies_identity() {
        ...
        assert!(
            exact_pull_request_association(command, "/repo", &Value::String(output), linkage(),)
                .is_some()
        );
        assert!(
            exact_pull_request_association(
