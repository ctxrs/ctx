...
use crate::model::{
    GitObjectFormat, GitObjectId, LOCAL_ROOT_AUTHORIZATION_FINGERPRINT_REVISION,
    MAX_COMMIT_OPERATION_MAPPINGS, RepositoryAlias, RepositoryBinding, RepositoryEvidence,
...
pub(super) const MAX_VERIFIED_COMMIT_OPERATION_OBJECTS: usize = MAX_COMMIT_OPERATION_MAPPINGS * 2;
...
impl EventProbeBudget {
    ...
    pub(super) fn start_cached_full_certification(&mut self) -> Result<(), ProbeFailure> {
        self.start_full_certification()
    }
...
impl GitSnapshot {
    fn into_certificate(
        ...
        let binding = RepositoryBinding {
            ...
            association_policy_revision: crate::model::REPOSITORY_ASSOCIATION_POLICY_REVISION,
        };
