...
use crate::{
    ...
    commands::{
        doctor::run_doctor_with_components as run_doctor,
        ...
        status::{
            malformed_config_failure_with_components as malformed_config_failure,
            removed_cloud_config_failure_with_components as removed_cloud_config_failure,
            run_status_authorized_with_components as run_status, run_usage_action,
        },
    },
...
mod classification;
#[cfg(test)]
use classification::command_local_usage_draft;
use classification::{
    command_can_report_malformed_config, command_daemon_autostart_trigger,
    command_is_status_report, command_json_output, command_machine_readable_output, quiet_output,
};
pub(crate) use classification::{
    command_deprecation_warning_eligible, command_operation_descriptor,
};
mod finalization;
mod parse;
...
#[derive(Debug, thiserror::Error)]
#[error("independent command completed")]
struct EngineExit(i32);
...
pub(crate) fn run() -> ExitCode {
    ...
    match run_cli() {
        Ok(()) => ExitCode::SUCCESS,
        Err(error) if error.is::<EngineExit>() => {
            std::process::exit(error.downcast_ref::<EngineExit>().map_or(1, |exit| exit.0));
        }
...
pub(crate) fn run_cli() -> Result<()> {
    ...
    cli.command = match cli.command {
        CommandRoot::Unified(command) => {
            ...
            return Err(EngineExit(status).into());
        }
        command => command,
    };
    ...
    cli.command = match cli.command {
        CommandRoot::Mcp(args) => {
            let data_root = cli.data_root.map(Ok).unwrap_or_else(default_data_root)?;
    ...
    let components = matches!(
        &cli.command,
        CommandRoot::Status(_) | CommandRoot::Doctor(_)
    )
    .then(|| match ctx_graph::optional_database(None) {
        Ok(database) => crate::unified_health::UnifiedHealth::inspect(database.as_deref()),
    ...
    let mut config =
        match AppConfig::load_with_deprecated_controls(&data_root, &deprecated_controls) {
            ...
            {
                return removed_cloud_config_failure(json_output, components.as_ref(), &mut ui);
            }
            Err(_) if command_is_status_report(&cli.command) => {
                return malformed_config_failure(json_output, components.as_ref(), &mut ui);
            }
            Err(_) if matches!(&cli.command, CommandRoot::Doctor(_)) => {
                return crate::commands::doctor::malformed_config_failure_with_components(
                    json_output,
                    components.as_ref(),
                    &mut ui,
                );
            ...
            Err(_)
                if matches!(
                    &cli.command,
                    CommandRoot::Docs(_) | CommandRoot::Integrations(_)
                ) =>
            {
                return run_config_independent_command(cli.command, &mut ui);
            }
...
/// Documentation and explicit agent setup remain usable if history configuration
/// is broken. No fallback configuration may silently re-enable telemetry.
fn run_config_independent_command(command: CommandRoot, ui: &mut Ui) -> Result<()> {
    let result = match command {
        CommandRoot::Docs(args) => docs::run(
            args,
            &mut DocsTelemetry::default(),
            &mut local_usage::CliUsage::excluded(),
            ui,
            &crate::Cli::command(),
        ),
        CommandRoot::Integrations(args) => integrations::run(
            ...
            &mut IntegrationTelemetry::default(),
            ui,
        ),
        _ => anyhow::bail!("command requires valid history configuration"),
    };
    ui.flush()
        .context("flush independent setup/documentation output")?;
    result
}
