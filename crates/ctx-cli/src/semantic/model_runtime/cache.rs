...
#[cfg(ctx_semantic_fastembed)]
pub(in crate::semantic) fn semantic_cpu_cache_snapshot(cache_dir: &Path) -> Result<PathBuf> {
...
#[cfg(ctx_semantic_fastembed)]
pub(in crate::semantic) fn semantic_cpu_cache_repairable(error: &anyhow::Error) -> bool {
    error
...
#[cfg(ctx_semantic_fastembed)]
pub(in crate::semantic) fn replace_cpu_model_cache_from_pinned_revision(
    cache_dir: &Path,
) -> Result<PathBuf> {
...
#[cfg(ctx_semantic_fastembed)]
pub(in crate::semantic) fn maybe_cleanup_semantic_cpu_download_cache_after_cached_acquisition(
    cache_dir: &Path,
    daemon_owned: bool,
) {
    if !daemon_owned {
