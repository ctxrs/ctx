...
#[cfg(ctx_semantic_fastembed)]
pub(in crate::semantic) fn acquire_cpu_fallback_backend(
    cache_dir: &Path,
    policy: SemanticEmbedPolicy,
    preference: BackendPreference,
    ...
    .or_else(|accelerator_model_error| {
        acquire_cpu_backend(cache_dir, policy, preference).with_context(|| {
            format!(
...
#[cfg(ctx_semantic_fastembed)]
pub(in crate::semantic) fn acquire_ort_backend(
    cache_dir: &Path,
    policy: SemanticEmbedPolicy,
    preference: BackendPreference,
    ...
    let (model, runtime_artifact_identity, windows_ml_registration) =
        load_cached_ort_model(&snapshot, cache_dir, &policy, kind, variant)?;
    Ok(SemanticEmbedder {
...
#[cfg(ctx_semantic_fastembed)]
pub(in crate::semantic) fn acquire_cpu_model_for_daemon(
    cache_dir: &Path,
...
#[cfg(ctx_semantic_fastembed)]
pub(in crate::semantic) fn acquire_accelerator_model_for_daemon(
    cache_dir: &Path,
    ...
    match installed_accelerator_runtime_identity(cache_dir, flavor)? {
...
pub(super) fn load_cached_ort_model(
    ...
    let (runtime_artifact_identity, loaded_accelerator) = if let Some(flavor) = accelerator_flavor {
        let runtime = ensure_semantic_accelerator_runtime_loaded(cache_dir, flavor).map_err(
            |error| -> anyhow::Error {
                SemanticProvisioningRequired {
                    asset: flavor.asset_name(),
                    detail: format!("{error:#}"),
        ...
        let path = ensure_semantic_onnxruntime_loaded(cache_dir)?;
        (
    ...
    if let Some((runtime, flavor)) = loaded_accelerator.as_ref() {
        ...
        ensure_semantic_onnxruntime_loaded(cache_dir)
...
pub(in crate::semantic) enum BackendPreference {
    Auto,
    Cpu,
    CoreMl,
    Cuda,
...
impl BackendPreference {
    ...
    pub(super) fn parse(value: Option<&str>) -> Result<Self> {
        match value.map(str::trim).filter(|value| !value.is_empty()) {
            ...
            Some("windowsml") => Ok(Self::WindowsMl),
            Some(value) => Err(anyhow!(
                "unsupported {SEMANTIC_BACKEND_PREFERENCE_ENV} value {value:?}; expected auto, cpu, coreml, cuda, or windowsml"
            )),
        }
    }
...
pub(crate) enum SemanticNativeAcceleratorTarget {
    CoreMl,
    Cuda,
    WindowsMl,
}
...
impl SemanticNativeAcceleratorTarget {
    ...
    #[allow(dead_code)] // Signed provisioning consumes this seam in a separate integration lane.
    pub(crate) fn runtime_platform(self) -> Option<&'static str> {
        match self {
            Self::CoreMl => None,
...
#[allow(dead_code)] // Signed provisioning consumes this seam in a separate integration lane.
pub(crate) fn semantic_native_accelerator_target() -> Option<SemanticNativeAcceleratorTarget> {
...
pub(crate) fn prepare_platform_semantic_acceleration(cache_dir: &Path) -> Result<Option<String>> {
...
pub(super) fn acquire_accelerator_backend(
    ...
    let acquired = acquire_ort_backend(
        cache_dir,
        semantic_embed_policy_for(SemanticComputeClass::Accelerator),
        preference,
        kind,
        kind,
    )
