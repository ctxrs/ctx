...
#[cfg(ctx_semantic_fastembed)]
#[test]
fn daemon_clean_cache_bootstrap_runs_semantic_before_history_refresh() -> Result<()> {
    let temp = tempfile::tempdir()?;
    ...
    assert!(
        !temp.path().join("semantic-model-cache").exists(),
        "scheduler test must start from a genuinely clean model cache"
    );
