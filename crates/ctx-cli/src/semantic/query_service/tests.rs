...
fn semantic_filter_is_applied_before_top_k_across_more_than_4096_candidates() -> Result<()> {
    ...
    let projection = index.semantic_filter_projection(&EventSearchFilters {
        workspace: Some("ONLY-target".to_owned()),
        ..EventSearchFilters::default()
    })?;
    assert_eq!(projection.len(), 1);

    let (candidates, diagnostics) = source_semantic_candidates_with_embedding(
        &index,
        &pinned,
        &projection,
        1,
...
fn normalized_test_embedding(first: f32, second: f32) -> Vec<f32> {
    let mut embedding = vec![0.0; super::super::model_contract::SEMANTIC_DIMENSIONS];
