...
#[test]
fn interrupted_incremental_retirement_converges_as_atomic_cleanup() {
    let temp = tempdir();
    let data_root = temp.path().join("data");
    ...
    assert!(attempts >= 3, "owner cleanup must remain bounded");
    let store = Store::open(&db_path).unwrap();
    assert!(!store.has_pending_provider_file_publications().unwrap());
    assert!(store.export_archive().unwrap().events.is_empty());
}
...
#[test]
fn retirement_discards_staged_candidates_and_converges_across_store_reopens() {
    let temp = tempdir();
    let data_root = temp.path().join("data");
