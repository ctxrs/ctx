...
fn phase_attribution(receipts: &[CodexSourceBackedIngestReceiptV0], query: Duration) -> Value {
    ...
    json!({
        ...
        "prefiltered_records": receipts.iter().map(|receipt| receipt.counters.prefiltered_records).sum::<u64>(),
        ...
        "emitted_pages": receipts.iter().map(|receipt| receipt.counters.emitted_pages).sum::<u64>(),
