...
fn recording_hot_path_benchmark_smoke() {
    ...
    samples.sort_unstable();
    let p99 = samples[989];
    eprintln!("local usage warm upsert p99 over 1,000 samples: {p99:?}");
    // Fastbuild/debug Bazel runs this beside hundreds of filesystem tests and
    // owns only a coarse runaway-I/O smoke ceiling. Release qualification owns
...
fn conversion_action_is_limited_to_trial_and_locked_access() {
    ...
    assert_eq!(trial["price"], "$15/month");
