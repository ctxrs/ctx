...
pub(crate) fn estimate_usage(facts: EstimateFacts) -> UsageEstimates {
    ...
    let search_seconds = facts
        ...
        .saturating_mul(ESTIMATE_MODEL.result_bearing_search_seconds);
    let open_seconds = facts
        .discovered_record_opens
        .saturating_mul(ESTIMATE_MODEL.discovered_record_open_seconds);
    let produced_seconds = facts
        .produced_blame_requests
        .saturating_mul(ESTIMATE_MODEL.produced_blame_seconds);
    let possible_seconds = facts
        .possible_blame_requests
        .saturating_mul(ESTIMATE_MODEL.possible_blame_seconds);
    UsageEstimates {
        model: ESTIMATE_MODEL,
        approximate_context_tokens,
        approximate_avoided_context_tokens,
        estimated_time_saved_seconds: search_seconds
            .saturating_add(open_seconds)
            .saturating_add(produced_seconds)
            .saturating_add(possible_seconds),
    }
}
...
fn covered_token_estimate(
    ...
    CoveredTokenEstimate {
        approximate_tokens: (coverage != EstimateCoverage::UnavailableLegacy).then(|| {
