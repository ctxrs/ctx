...
fn downgrade_certified_cursor(
    ...
    let parser = cursor["p"].as_u64().unwrap();
    let policy = cursor["o"].as_u64().unwrap();
...
fn certified_cursor_revisions(database: &Path, provider: &str) -> (u64, u64) {
    ...
    (cursor["p"].as_u64().unwrap(), cursor["o"].as_u64().unwrap())
}
...
fn codex_automatic_and_explicit_same_spelling_share_ids_and_cursor_streams() {
    ...
    assert_eq!(automatic["totals"]["imported_events"], 2, "{automatic:#}");
...
fn explicit_codex_path_repairs_a_legacy_cursor_once() {
    ...
    connection
        .execute(
            "DELETE FROM events
             WHERE event_type = 'command_output'
               AND json_extract(payload_json, '$.body.call_id') = ?1",
