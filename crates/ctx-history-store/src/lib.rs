...
impl Store {
    ...
    pub fn migrate(&self) -> Result<()> {
        ...
        if user_version < 38 {
            migrate_to_v38(&self.conn)?;
        }
        if user_version < 39 {
            migrate_to_v39(&self.conn)?;
        }
        if user_version < 40 {
            migrate_to_v40(&self.conn)?;
        }
        if user_version < 41 {
            migrate_to_v41(&self.conn)?;
        }
        if user_version < 42 {
...
mod catalog_tests {
    ...
    #[test]
    fn schema_v41_adds_provider_checks() {
        for (provider, source_format, source_root, source_path) in [
            (
                "codearts_agent",
                "codearts_agent_kernel_sqlite",
                "/tmp/codearts/opencode.db",
                "/tmp/codearts/opencode.db",
            ),
            (
                "zencoder",
