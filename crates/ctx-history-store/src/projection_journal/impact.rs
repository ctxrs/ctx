...
impl Store {
    ...
    pub(crate) fn journal_file_touch_mutated(&self, id: Uuid) -> Result<()> {
        ...
        append_entity(&self.conn, JournalEntityKind::FileTouch, id)
    }
    ...
    pub(crate) fn journal_vcs_change_mutated(&self, id: Uuid) -> Result<()> {
        ...
        append_entity(&self.conn, JournalEntityKind::VcsChange, id)
    }
    ...
    pub(crate) fn journal_history_link_mutated(
        ...
        history_link_mutated(&self.conn, target_type, target_id)
    }
}

fn event_mutated(conn: &rusqlite::Connection, id: Uuid) -> Result<()> {
...
fn source_mutated(conn: &rusqlite::Connection, id: Uuid) -> Result<()> {
    append_query_when_active(
        conn,
...
fn session_mutated(conn: &rusqlite::Connection, id: Uuid) -> Result<()> {
    append_query_when_active(
        conn,
        JournalEntityKind::Event,
...
fn run_mutated(conn: &rusqlite::Connection, id: Uuid) -> Result<()> {
    append_query_when_active(
        conn,
        JournalEntityKind::Event,
...
fn history_link_mutated(
    ...
    if target_type == "vcs_change" {
        append_entity(conn, JournalEntityKind::VcsChange, target_id)?;
    }
    Ok(())
}
...
pub(super) fn append_query_when_active(
    ...
    for entity_id in query_ids(conn, sql, id)? {
        append_entity(conn, kind, entity_id)?;
    }
    Ok(())
}
