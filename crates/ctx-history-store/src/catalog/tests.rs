...
use rusqlite::{ffi::ErrorCode, params, params_from_iter, types::Value as SqlValue};
use uuid::Uuid;

use super::source_files::{
    source_import_file_from_row, source_import_file_select_sql, source_import_pending_page_sql,
};
use super::source_reconciliation::{
...
fn complete_inventory_control(
    provider: CaptureProvider,
    source_format: &str,
    source_root: &str,
    observed_at_ms: i64,
) -> SourceImportFile {
    SourceImportInventoryControl::new(provider, source_format, source_root, 0, 0, observed_at_ms)
        .complete_file(None, 0, 0)
        .unwrap()
}
...
fn assert_observation_conflict(
    ...
    match result.unwrap_err() {
        StoreError::SourceImportObservationConflict {
            operation: actual_operation,
            provider: actual_provider,
            source_path: actual_source_path,
        } => {
            assert_eq!(actual_operation, operation);
            assert_eq!(actual_provider, provider.as_str());
            assert_eq!(actual_source_path, source_path);
        }
        error => panic!("expected observation conflict, got {error:?}"),
    }
}
