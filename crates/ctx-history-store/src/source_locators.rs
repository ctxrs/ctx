...
impl Store {
    /// Resolves the canonical source identity without changing locator or route
    /// authority. A later NativePath publication group must reconcile the same
    /// observation before publishing dependent rows or cursors.
    pub fn plan_provider_source_locator(
        &self,
        ...
        if let Some(exact) = self.provider_source_locator_by_identity(observation)? {
            ...
            if exact.canonical_source_identity != observation.proposed_source_identity
                || self.provider_source_route_count(observation, &exact.alias_group_identity)? != 0
            {
                return Err(StoreError::ProviderSourceRelocationAmbiguous {
                    provider: observation.provider.as_str().to_owned(),
                    source_format: observation.source_format.clone(),
        ...
        if let [candidate] = candidates.as_slice() {
            if exact_locator_is_missing(candidate.raw_source_path.as_deref()) {
    ...
    pub(crate) fn reconcile_provider_source_locator_tx(
        ...
        if let Some(exact) = self.provider_source_locator_by_identity(observation)? {
            ...
            let Some(current) =
                ...
                let stale_route_count =
                    self.provider_source_route_count(observation, &exact.alias_group_identity)?;
    ...
    fn provider_source_route_count(
        &self,
        observation: &ProviderSourceLocatorObservation,
        alias_group_identity: &str,
    ) -> Result<i64> {
