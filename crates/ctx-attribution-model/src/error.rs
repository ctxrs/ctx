...
impl ProtocolError {
    ...
    ///
    /// Generic validation errors may continue to use a null `details` field.
...
mod tests {
    ...
    fn retryability_and_typed_details_round_trip_without_helper_message_parsing() {
        let error = ProtocolError::new(ErrorClass::Ambiguous, "diagnostic detail")
            .with_retryable(true)
        ...
        assert_eq!(encoded["message"], "diagnostic detail");
        assert_eq!(
    ...
    fn reason_class_and_candidate_kind_matrix_is_closed() {
        ...
        ProtocolError::new(ErrorClass::Ambiguous, "detail")
            ...
            .unwrap();
        assert!(
            ProtocolError::new(ErrorClass::ResourceNotFound, "detail")
    ...
    fn blame_boundary_requires_typed_details_without_changing_generic_errors() {
        for class in [
            ...
            let generic = ProtocolError::new(class, "diagnostic detail");
            generic.validate().unwrap();
            generic.validate_blame_details().unwrap_err();
        }

        ProtocolError::new(ErrorClass::Internal, "diagnostic detail")
            .validate_blame_details()
            .unwrap();
    }
}
