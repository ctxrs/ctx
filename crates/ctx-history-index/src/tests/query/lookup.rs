...
fn escape_heavy_metadata_round_trips_from_the_single_stored_core_record() {
    ...
    assert!(index.searcher.schema().get_field("query_metadata").is_err());
}
...
fn strict_core_event_batch_materializes_at_most_one_encoded_heavy_record_before_declining() {
    ...
    let records = (1..=3)
        .map(|sequence| {
            ...
            record.branch = Some(escaped_metadata.clone());
            record.validate_contract().unwrap();
...
fn bounded_session_coordinate_queries_ignore_pathological_nonselected_cardinality() {
    ...
    let mut appending = GenerationWriter::open(temp.path(), WriterOptions::default()).unwrap();
    appending
        .writer_mut()
        .unwrap()
        .set_merge_policy(Box::<NoMergePolicy>::default());
...
fn session_event_budget_declines_before_materializing_an_oversized_session() {
    ...
    assert_eq!(crate::query::stored_event_record_materializations(), 3);
    let core = index
