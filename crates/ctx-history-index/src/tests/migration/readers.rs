...
fn subprocess_continuous_readers_query_across_atomic_predecessor_swap() {
    ...
    let predecessor_reads = std::sync::Arc::new(std::sync::atomic::AtomicUsize::new(0));
    let successor_reads = std::sync::Arc::new(std::sync::atomic::AtomicUsize::new(0));
    let reader_stop = std::sync::Arc::clone(&stop);
    let reader_predecessor_reads = std::sync::Arc::clone(&predecessor_reads);
    let reader_successor_reads = std::sync::Arc::clone(&successor_reads);
    let reader = thread::spawn(move || {
        while !reader_stop.load(std::sync::atomic::Ordering::Acquire) {
            ...
            if index.uses_allowlisted_predecessor_contract() {
                reader_predecessor_reads.fetch_add(1, std::sync::atomic::Ordering::AcqRel);
            } else {
                reader_successor_reads.fetch_add(1, std::sync::atomic::Ordering::AcqRel);
            }
        }
    });

    let initial_deadline = Instant::now() + SUBPROCESS_TIMEOUT;
    while predecessor_reads.load(std::sync::atomic::Ordering::Acquire) < 4 {
        assert!(
    ...
    let before_continue = predecessor_reads.load(std::sync::atomic::Ordering::Acquire);
    let during_deadline = Instant::now() + SUBPROCESS_TIMEOUT;
    while predecessor_reads.load(std::sync::atomic::Ordering::Acquire) < before_continue + 4 {
        assert!(
    ...
    while successor_reads.load(std::sync::atomic::Ordering::Acquire) < 4 {
        assert!(
