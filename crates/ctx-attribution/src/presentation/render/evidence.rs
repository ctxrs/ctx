use ctx_attribution_model::evidence_preview::{
...
use ctx_terminal::ui::{
    ColorMode, StreamKind, TestContext, sanitize_untrusted_history_body_for_terminal,
};
use ctx_terminal::ui::{Document, Line, RenderContext, Span, Token};
use unicode_segmentation::UnicodeSegmentation as _;

use super::layout::{
    FIELD_GAP, display_width, enum_heading, line_range_text, push_atomic, push_authored,
    push_field, push_heading, timestamp_text,
};
...
fn preview_item(
    ...
    if let Some(event_occurred_at_ms) = preview.event_occurred_at_ms {
        push_field(
            ...
            "Event time",
            10,
            &timestamp_text(context, event_occurred_at_ms),
            Token::Text,
            true,
        );
    }
