use ctx_attribution_model::{BlameMatch, PullRequestBlameMatch, PullRequestBlameRelationship};

use ctx_terminal::ui::{Document, RenderContext, Token};

use super::{
    layout::{
        METADATA_LABEL_WIDTH, confidence_token, enum_heading, push_authored, push_enum_field,
        push_field, push_heading, push_references, push_resource_primary, push_role_resource,
        state_token, timestamp_text,
    },
...
fn render_activity(
    ...
    if let Some(time) = activity.fact_occurred_at_ms {
        let time = timestamp_text(context, time);
        push_field(
            document,
            context,
            6,
