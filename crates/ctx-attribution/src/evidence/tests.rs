use std::{cell::Cell, fs, path::Path, process::Command, sync::OnceLock};
...
use ctx_history_core::{
    ActivityInvocation, ActivityJsonCapture, CORE_ACTIVITY_REVISION, CertifiedSource, CoreActivity,
    CoreRecord, EventIdentityInput, LiteralFactKind, NativeItemKey, NativeSessionKey,
    ProviderDeclaredFact, ScannedSourceCounts, SessionIdentityInput, SourceAnchor, SourceKey,
    SourceObservation, TypedKey, derive_event_id, derive_session_id,
};
use ctx_history_index::{GenerationWriter, IndexError, WriterOptions};
use sha2::{Digest as _, Sha256};

use super::*;

const PATH: &str = "src/lib.rs";
const INVOCATION_UNIT: &str = r#"{"path":"src/lib.rs"}"#;
...
fn repository_root() -> &'static Path {
    ...
    ROOT.get_or_init(|| {
        ...
        fs::write(root.path().join(PATH), "fn fixture() {}\n").unwrap();
        for arguments in [
            &["init", "-q"][..],
...
pub(crate) fn core_record(seed: u8, sequence: u64, body: impl Into<String>) -> CoreRecord {
    let body = body.into();
    let body = if body.contains(INVOCATION_UNIT) {
        body
    } else {
        format!("{body}\n{INVOCATION_UNIT}")
    };
    ...
    record.content.activity = Some(CoreActivity {
        ...
        provider_call_id: Some(TypedKey::utf8(format!("call-{sequence}")).unwrap()),
        ...
        facts: vec![
            ProviderDeclaredFact {
                ...
                value: repository_root().to_string_lossy().into_owned(),
            },
            ProviderDeclaredFact {
...
pub(crate) fn publish(index_root: &std::path::Path, record: &CoreRecord, revision: u8) -> String {
...
fn one_strict_bounded_batch_deduplicates_ids_and_caps_citations() {
    ...
    let model = hydrate_evidence_previews_with(&result, |requested_generation, ids, budget| {
        calls.set(calls.get() + 1);
        assert_eq!(requested_generation, generation);
        assert_eq!(ids, &[first.event_id, second.event_id]);
...
fn malformed_mixed_or_ranged_citations_stop_before_index_open() {
    ...
    for evidence in cases {
        let model = hydrate_evidence_previews_with(&result(evidence), |_, _, _| {
...
fn read_errors_missing_records_wrong_order_and_wrong_generation_fail_closed() {
    ...
    let race = hydrate_evidence_previews_with(&result, |_, _, _| {
        Err(anyhow!(IndexError::ConcurrentGenerationChange))
    });
    assert!(race.previews.is_empty());

    let missing = hydrate_evidence_previews_with(&result, |_, _, _| {
        Ok(Some(HydratedEvidenceBatch {
            generation_id: generation.clone(),
    ...
    let wrong_order = hydrate_evidence_previews_with(&result, |_, _, _| {
        Ok(Some(HydratedEvidenceBatch {
            generation_id: generation.clone(),
    ...
    let wrong_generation = hydrate_evidence_previews_with(&result, |_, _, _| {
        Ok(Some(HydratedEvidenceBatch {
...
fn digest_and_every_event_coordinate_mismatch_are_omitted() {
    ...
    let model = hydrate_evidence_previews_with(&result(vec![bad_digest]), |_, _, _| {
        Ok(Some(HydratedEvidenceBatch {
            generation_id: generation.clone(),
    ...
    for mismatch in 0..4 {
        ...
        let other = core_record(8, 8, "modified: src/lib.rs");
        match mismatch {
            0 => hydrated.source = other.source.clone(),
            1 => hydrated.session_id = other.session_id,
            2 => hydrated.event_id = other.event_id,
            3 => hydrated.event_sequence += 1,
            _ => unreachable!(),
        }
        let model = hydrate_evidence_previews_with(&result(vec![cited]), |_, _, _| {
            Ok(Some(HydratedEvidenceBatch {
                generation_id: generation.clone(),
...
fn non_file_context_is_not_applicable_without_any_core_read() {
    ...
    for target in [
        ...
        let model = hydrate_evidence_previews_with(&result, |_, _, _| {
...
fn active_and_retained_previous_generations_hydrate_from_real_index() {
    ...
    assert_eq!(previous_model.previews[0].excerpt, INVOCATION_UNIT);
}
...
fn compact_eval_envelope_fits_and_mixed_oversized_records_fail_closed() {
    ...
    assert_eq!(
        ...
        EvidenceHydrationBudget {
            aggregate: SnapshotPageBudget::new(3 * 64 * 1_024, 3 * 64 * 1_024),
            per_record: SnapshotPageBudget::new(64 * 1_024, 64 * 1_024),
        }
    );
}
