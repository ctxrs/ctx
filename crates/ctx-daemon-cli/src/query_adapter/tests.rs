...
struct ExactCompletionGate {
    pauses: Cell<u32>,
}

impl ExactCompletionGate {
    fn pause(&self) {
        self.pauses.set(self.pauses.get() + 1);
    }

    fn assert_pauses(&self, expected: u32) {
        assert_eq!(self.pauses.get(), expected);
    }
}

#[test]
fn exact_daemon_semantic_completion_waits_for_a_delayed_matching_acknowledgement() -> Result<()> {
    let temp = tempfile::tempdir()?;
    let index_root = ctx_history_refresh::source_backed_index_root(temp.path());
    let (index, _) = semantic_index_revision_at(&index_root, 1, true)?;
    let generation = index.generation_id().to_owned();
    let reconciliation_index = VerifiedIndex::open_pinned(&index_root)?;
    let gate = ExactCompletionGate {
        pauses: Cell::new(0),
    };

    let pin = wait_for_exact_daemon_semantic_completion_with(
        PinnedSourceBackedGeneration::from_index(index),
        || Ok(generation.clone()),
        || Ok(None),
        |pin| {
            SemanticQueryPin::preflight(
                pin.verified_index(),
                temp.path(),
                semantic_model_contract(),
            )
    ...
    gate.assert_pauses(1);
    assert_eq!(pin.generation_id(), generation);
    Ok(())
}

#[test]
fn exact_daemon_semantic_completion_reports_a_matching_failed_job() -> Result<()> {
    let temp = tempfile::tempdir()?;
    let (index, _) = semantic_index_revision(temp.path(), 1, false)?;
    let gate = ExactCompletionGate {
        pauses: Cell::new(0),
    };
    let expected_generation = index.generation_id().to_owned();
    let job_generation = expected_generation.clone();
    let Err(error) = wait_for_exact_daemon_semantic_completion_with(
        PinnedSourceBackedGeneration::from_index(index),
        || Ok(expected_generation.clone()),
        || {
            Ok(Some(json!({
                "core_generation_id": job_generation,
                "status": "failed",
                "retryable": true,
        ...
        |_| panic!("a matching failed daemon job must stop before semantic preflight"),
        |_| gate.pause(),
    ) else {
        panic!("a matching failed daemon job must be terminal");
    };
    ...
    gate.assert_pauses(0);
    Ok(())
}

#[test]
fn exact_daemon_semantic_completion_ignores_a_stale_failed_job() -> Result<()> {
    let temp = tempfile::tempdir()?;
    let (index, _) = semantic_index_revision(temp.path(), 1, false)?;
    let generation = index.generation_id().to_owned();
    let semantic_path = source_backed_semantic_vector_path(temp.path());
    ...
    let pin = wait_for_exact_daemon_semantic_completion_with(
        ...
        || {
            Ok(Some(json!({
                "core_generation_id": "stale-generation",
                "status": "failed",
                "retryable": false,
...
#[test]
fn exact_daemon_semantic_completion_reports_core_supersession_retryably() -> Result<()> {
    let temp = tempfile::tempdir()?;
    let (index, _) = semantic_index_revision(temp.path(), 1, false)?;
    let expected_generation = index.generation_id().to_owned();
    let gate = ExactCompletionGate {
    ...
    let Err(error) = wait_for_exact_daemon_semantic_completion_with(
        PinnedSourceBackedGeneration::from_index(index),
        || Ok("newer-generation".to_owned()),
        || panic!("supersession must stop before reading the semantic job"),
        |_| panic!("supersession must stop before semantic preflight"),
        |_| gate.pause(),
    ) else {
        panic!("a superseded pin must not be repinned");
    };
    ...
    assert!(matches!(
        ...
        ExactDaemonSemanticCompletionError::CoreSuperseded {
            ...
            retryable: true,
        } if generation_id == expected_generation && active_generation_id == "newer-generation"
    ));
    gate.assert_pauses(0);
    Ok(())
}
...
#[test]
fn exact_daemon_semantic_completion_is_a_noop_for_an_already_ready_generation() -> Result<()> {
    let temp = tempfile::tempdir()?;
    let (index, _) = semantic_index_revision(temp.path(), 1, false)?;
    let generation = index.generation_id().to_owned();
    let semantic_path = source_backed_semantic_vector_path(temp.path());
