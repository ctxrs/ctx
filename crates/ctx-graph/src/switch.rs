...
const RECEIPT: &str = "ctx-switch-graphify.json";
...
fn verify(db: &Path, exe: &Path, nodes: usize, edges: usize) -> Result<()> {
    ...
    drop(store);
    verify_mcp(db, exe, nodes, edges).context("ctx graph MCP verification failed")
}
...
fn verify_mcp(db: &Path, exe: &Path, nodes: usize, edges: usize) -> Result<()> {
    ...
    let result = (|| -> Result<()> {
        ...
        request(
            json!({"jsonrpc":"2.0","id":1,"method":"initialize","params":{"protocolVersion":"2024-11-05","capabilities":{},"clientInfo":{"name":"ctx-graph-switch","version":env!("CARGO_PKG_VERSION")}}}),
            1,
        )?;
...
pub fn run(args: SwitchArgs) -> Result<Report> {
    ...
    if let Some(bytes) = existing {
        let mut record: Receipt =
            serde_json::from_slice(&bytes).context("invalid ctx graph migration record")?;
        ensure!(
        ...
        let status = if args.undo {
            ...
            "undone"
                ...
                "migration uses a different ctx executable; run the original executable or undo before changing it"
            );
            ensure!(
...
fn lock(directory: &Path) -> Result<File> {
    ...
    let lock = OpenOptions::new()
        ...
        .open(path)?;
    lock.try_lock()
        .context("another ctx graph migration is running")?;
    Ok(lock)
}
