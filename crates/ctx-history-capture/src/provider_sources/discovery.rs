...
use super::{
    ...
    revisions::{provider_import_revision, PROVIDER_IMPORT_REVISIONS},
...
pub fn provider_source_for_persisted_format(
    provider: CaptureProvider,
    path: PathBuf,
    source_format: &str,
) -> Option<ProviderSource> {
    let source_format = PROVIDER_IMPORT_REVISIONS
        .iter()
    ...
    source.import_revision = provider_import_revision(provider, source_format);
    source.import_unit = provider_import_unit_spec(source_format);
    source.mutation_contract = provider_file_mutation_contract(provider, source_format);
    Some(source)
}

include!("discovery/source_from_location.rs");
