...
use super::row_position::{
    decode_shelley_position, encode_shelley_position, shelley_locator,
    shelley_oversize_session_locator, ShelleyCapturePhase, ShelleyKeyset,
};
use super::source::{
...
pub(crate) fn shelley_message_values(
    row: &rusqlite::Row<'_>,
...
fn shelley_conversation_values_at(
    ...
    Ok(vec![
        ...
        shelley_optional_text_value(row.get(offset + 2)?),
        shelley_optional_integer_value(row.get(offset + 3)?),
        shelley_optional_text_value(row.get(offset + 4)?),
        shelley_optional_text_value(row.get(offset + 5)?),
        shelley_optional_text_value(row.get(offset + 6)?),
        shelley_optional_integer_value(row.get(offset + 7)?),
        shelley_optional_text_value(row.get(offset + 8)?),
        shelley_optional_text_value(row.get(offset + 9)?),
        shelley_optional_text_value(row.get(offset + 10)?),
        shelley_optional_integer_value(row.get(offset + 11)?),
        shelley_optional_integer_value(row.get(offset + 12)?),
        shelley_optional_text_value(row.get(offset + 13)?),
        shelley_optional_integer_value(row.get(offset + 14)?),
        shelley_optional_text_value(row.get(offset + 15)?),
        shelley_optional_text_value(row.get(offset + 16)?),
    ])
}
