...
fn select_schema_family(
    ...
    session_message: Option<(OpenCodeNativeSchemaFamily, bool, bool)>,
    session_entry: Option<(OpenCodeNativeSchemaFamily, bool, bool)>,
    message: Option<(bool, bool)>,
    part: Option<(bool, bool)>,
    message_part_join: bool,
) -> Result<(OpenCodeNativeSchemaFamily, bool)> {
    let mut populated = Vec::new();
    if let Some((family, has_type, true)) = session_message {
        populated.push((family, has_type));
    }
    if let Some((family, has_type, true)) = session_entry {
        populated.push((family, has_type));
    }
