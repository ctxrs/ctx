...
#[test]
fn gemini_nativepath_result_identity_is_stable_and_fallback_is_content_sensitive() {
    let temp = TempDir::new().unwrap();
    let root = fixture_root(&temp);
    let path = transcript_path(&root);
    fs::create_dir_all(path.parent().unwrap()).unwrap();

    let scan = |records: &[Value]| {
        ...
        while let Some(mut page) = reader.next_page().unwrap() {
            ...
            outputs.extend(page.output_pages.drain(..).flat_map(|page| page.outputs));
        }
        (events, outputs)
    };

    let stable_record = |content: &str| {
        json!({
            "id": "result-record",
            "type": "gemini",
            "toolCalls": [{
                "id": "call-stable",
                "name": "run_shell_command",
                "result": {"content": content, "error": true}
            }]
        })
    };
    let (first_events, first_outputs) =
        scan(&[header("stable-result", "main"), stable_record("first")]);
    let (rewritten_events, rewritten_outputs) = scan(&[
        header("stable-result", "main"),
        json!({"type": "metadata", "value": "position shift"}),
        stable_record("rewritten"),
    ]);
    assert_eq!(first_events.len(), 1);
    assert_eq!(first_outputs.len(), 1);
    assert_eq!(rewritten_events.len(), 1);
    assert_eq!(rewritten_outputs.len(), 1);
    assert_eq!(first_events[0].identity, rewritten_events[0].identity);
    assert_eq!(
        first_outputs[0].coordinate.unit_key,
        rewritten_outputs[0].coordinate.unit_key
    );
    assert_ne!(first_events[0].body_sha256, rewritten_events[0].body_sha256);
    assert_eq!(first_outputs[0].content, b"first");
    assert_eq!(rewritten_outputs[0].content, b"rewritten");

    let fallback_record = |content: &str| {
        json!({
            "type": "gemini",
            "toolCalls": [{
                "result": {"content": content, "error": true}
    ...
    let (fallback_first, fallback_first_outputs) =
        scan(&[header("fallback-result", "main"), fallback_record("same")]);
    let (fallback_shifted, fallback_shifted_outputs) = scan(&[
        header("fallback-result", "main"),
        json!({"type": "metadata", "value": "position shift"}),
        fallback_record("same"),
    ]);
    let (fallback_changed, fallback_changed_outputs) = scan(&[
        header("fallback-result", "main"),
        fallback_record("changed"),
    ]);
    assert_eq!(fallback_first[0].identity, fallback_shifted[0].identity);
    assert_eq!(
        fallback_first_outputs[0].coordinate.unit_key,
        fallback_shifted_outputs[0].coordinate.unit_key
    );
    assert_ne!(fallback_first[0].identity, fallback_changed[0].identity);
    assert_ne!(
        fallback_first_outputs[0].coordinate.unit_key,
        fallback_changed_outputs[0].coordinate.unit_key
    );
}

#[test]
fn gemini_nativepath_dedupes_duplicate_result_subrecord_ids() {
    let temp = TempDir::new().unwrap();
    let root = fixture_root(&temp);
    let path = write_transcript(
        &root,
        &[
            header("duplicate-result-subrecord", "main"),
            json!({
                "id": "result-record",
                "type": "gemini",
                "toolCalls": [
                    {
                        "id": "duplicate-call",
                        "result": {"content": "first wins", "error": true}
                    },
                    {
                        "id": "duplicate-call",
                        "result": {"content": "second is deduped", "error": true}
                    }
                ]
            }),
        ],
    ...
    assert_eq!(outputs[0].content, b"first wins");
