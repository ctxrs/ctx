...
pub(super) const MAX_STRICT_FILE_TARGETS: usize = 64;
...
pub(super) fn strict_tool_call_projection(
    ...
    let normalized_body = serde_json::to_string(block)?;
    let Some(operation_ordinal) = u32::try_from(invocation_ordinal).ok() else {
        return Ok(abstained(
            normalized_body,
            StrictInvocationAbstention::Capacity,
        ));
    };
    let normalized_text_range = match strict_text_range(0, normalized_body.len()) {
        Some(range) => range,
        None => {
            return Ok(abstained(
                normalized_body,
    ...
    let extraction = strict_file_invocations(block, operation_ordinal, normalized_text_range);
    Ok(match extraction {
        StrictExtraction::Exact(file_invocations) => StrictToolCallProjection {
            normalized_body,
            file_invocations,
            abstention: None,
        },
        StrictExtraction::NoTarget => StrictToolCallProjection {
            normalized_body,
            file_invocations: Vec::new(),
            abstention: None,
        },
        StrictExtraction::Abstained(reason) => abstained(normalized_body, reason),
    })
}

fn abstained(
    normalized_body: String,
    reason: StrictInvocationAbstention,
) -> StrictToolCallProjection {
    StrictToolCallProjection {
        normalized_body,
        file_invocations: Vec::new(),
...
enum StrictExtraction {
    Exact(Vec<UnscopedRepositoryFileInvocationEvidence>),
    NoTarget,
    Abstained(StrictInvocationAbstention),
}

fn strict_file_invocations(
    block: &Value,
    operation_ordinal: u32,
    normalized_text_range: RepositoryFileInvocationTextRange,
) -> StrictExtraction {
    ...
    let targets = match targets {
        StrictTargets::Exact(targets) => targets,
        StrictTargets::Opaque => {
            return StrictExtraction::Abstained(StrictInvocationAbstention::Opaque)
        }
        StrictTargets::Capacity => {
            return StrictExtraction::Abstained(StrictInvocationAbstention::Capacity)
        }
    };
        ...
        targets
            ...
            .map(
                |(path, prior_path)| UnscopedRepositoryFileInvocationEvidence {
                    ...
                    normalized_text_range: Some(normalized_text_range),
                },
            )
            .collect(),
    )
...
fn exactly_one_alias_string(
    ...
    Ok(Some(
        exact_path(value).ok_or(StrictTargets::Opaque)?.to_owned(),
    ))
}
...
fn strict_path_array(value: Option<&Value>) -> Result<Option<Vec<String>>, StrictTargets> {
    ...
    if values.len() > MAX_STRICT_FILE_TARGETS {
...
fn checked_targets(targets: Vec<(String, Option<String>)>) -> StrictTargets {
    ...
    if targets.len() > MAX_STRICT_FILE_TARGETS {
...
fn exact_path(value: &Value) -> Option<&str> {
    value.as_str().and_then(bounded_path)
}
