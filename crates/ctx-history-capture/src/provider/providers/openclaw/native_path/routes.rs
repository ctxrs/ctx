...
#[cfg(test)]
pub(in super::super) fn install_released_cursor_for_test(
    store: &mut Store,
    machine_id: &str,
    path: &Path,
) -> Result<()> {
    ...
    let mut stored =
        store
            ...
            .ok_or(CaptureError::SystemInvariant(
                "OpenClaw test fixture has no committed cursor",
            ))?;
    let committed =
        decode_native_path_committed_cursor(&stored.cursor).map_err(CaptureError::from)?;
    let wire: CursorWire = serde_json::from_str(committed.provider_cursor())?;
    let checkpoint = wire.checkpoint;
    let legacy = ReleasedParserCheckpoint {
        session: ReleasedSessionState {
            provider_session_id: checkpoint.session.provider_session_id,
            agent_id: checkpoint.session.agent_id,
            started_at: checkpoint.session.started_at,
            cwd: checkpoint.session.cwd,
            index_revision: checkpoint.source_observation.index_revision,
        },
        next_ordinal: checkpoint.next_raw_ordinal,
        header_anchor: checkpoint.session.header_anchor,
        emitted_session: true,
        accepted_events: checkpoint.accepted_events,
    };
    let position = released_jsonl_position_for_test(path, checkpoint.complete_prefix_end)?;
    stored.cursor = CertifiedProviderCursor::new(
        observation.source_revision(),
        OPENCLAW_RELEASED_CAPTURE_REVISION,
        OPENCLAW_RELEASED_POLICY_REVISION,
        position,
        crate::provider::importer::BoundedParserCheckpoint::from_serializable(&legacy)?,
    )?
    .with_rejected_records(checkpoint.rejected_records)
    .encode()?;
    store.upsert_sync_cursor(&stored)?;
    Ok(())
}

#[cfg(test)]
pub(in super::super) fn committed_generation_for_test(
    store: &Store,
    machine_id: &str,
    path: &Path,
) -> Result<u64> {
    ...
    let committed =
        decode_native_path_committed_cursor(&stored.cursor).map_err(CaptureError::from)?;
    let wire: CursorWire = serde_json::from_str(committed.provider_cursor())?;
    Ok(wire.checkpoint.generation)
}

#[cfg(test)]
pub(super) fn released_jsonl_position_for_test(
    path: &Path,
    offset: u64,
) -> Result<crate::native_source::NativePosition> {
    let proof_length = offset.min(RELEASED_BOUNDARY_MAX_BYTES);
    let proof_start = offset.saturating_sub(proof_length);
    let mut proof = vec![
        0_u8;
        usize::try_from(proof_length).map_err(
            |_| CaptureError::SystemInvariant("released OpenClaw test proof length exceeds usize",)
        )?
    ];
    let mut file = File::open(path)?;
    file.seek(SeekFrom::Start(proof_start))?;
    file.read_exact(&mut proof)?;
    let proof_length = u32::try_from(proof_length).map_err(|_| {
        CaptureError::SystemInvariant("released OpenClaw test proof length exceeds u32")
    })?;
    ...
    hasher.update(&proof);
    let mut encoded = Vec::with_capacity(RELEASED_POSITION_ENCODED_BYTES);
    ...
    crate::native_source::NativePosition::new("jsonl-byte-boundary-v1", encoded)
