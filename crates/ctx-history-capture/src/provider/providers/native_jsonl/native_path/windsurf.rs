...
const WINDSURF_OUTPUT_FRONTIER_VERSION: u32 = 1;
const WINDSURF_OUTPUT_PARSER_REVISION: &str = "windsurf-direct-native-jsonl-v1";
...
pub(crate) fn import_windsurf_nativepath_tree(
    ...
    let known_routes = known_windsurf_routes(store, &request.machine_id, &configured_source_root)?;
...
struct WindsurfInventory {
    paths: BTreeSet<PathBuf>,
    root_missing: bool,
}

fn discover_live_transcripts(root: &Path) -> Result<WindsurfInventory> {
    match std::fs::symlink_metadata(root) {
        Ok(_) => {}
        Err(error) if error.kind() == io::ErrorKind::NotFound => {
            return Ok(WindsurfInventory {
                paths: BTreeSet::new(),
                root_missing: true,
            });
        }
    ...
    super::super::traversal::visit_jsonl_tree_files(
        root,
        &|path| {
            super::super::dialect::native_jsonl_file_is_selected(CaptureProvider::Windsurf, path)
        },
        &mut |path| {
...
fn known_windsurf_routes(
    store: &Store,
    machine_id: &str,
    source_root: &Path,
) -> Result<Vec<KnownWindsurfRoute>> {
    let source_root = source_root.display().to_string();
    let mut routes = BTreeMap::<String, KnownWindsurfRoute>::new();
    for source in store.list_capture_sources()? {
        if source.descriptor.provider != CaptureProvider::Windsurf
            || source.descriptor.machine_id != machine_id
            || source.descriptor.source_format.as_deref()
                != Some(WINDSURF_CASCADE_HOOK_TRANSCRIPT_SOURCE_FORMAT)
        ...
        if routes.insert(locator_identity, route).is_some() {
            return Err(CaptureError::SystemInvariant(
                "Windsurf persisted duplicate current routes for one transcript",
            ));
        }
    }
...
fn retire_missing_routes(
    ...
    known_routes: &[KnownWindsurfRoute],
...
fn retire_route(
    ...
    let retirement = ProviderSourceRouteRetirement {
        ...
        source_format: WINDSURF_CASCADE_HOOK_TRANSCRIPT_SOURCE_FORMAT.to_owned(),
...
fn replay_windsurf_source(
    ...
    let progress_cursor = progress
        ...
        .filter(|cursor| cursor.version == WINDSURF_OUTPUT_FRONTIER_VERSION)
        .and_then(|cursor| serde_json::from_slice::<DirectJsonlCheckpoint>(&cursor.payload).ok())
        .filter(|checkpoint| {
            checkpoint.is_supported_for(
                CaptureProvider::Windsurf,
                WINDSURF_CASCADE_HOOK_TRANSCRIPT_SOURCE_FORMAT,
            )
        });
    let can_resume = progress.as_ref().is_some_and(|progress| {
        progress.parser_revision == WINDSURF_OUTPUT_PARSER_REVISION
    ...
    let mut output_state = WindsurfOutputState::new(
        output_source,
        progress,
        source_change,
        can_resume,
    ...
    while let Some(page) = reader.next_page()? {
        if !direct_jsonl_checkpoint_is_covered_by(authority, &page.next_checkpoint) {
            return Err(CaptureError::InvalidPayload(
                "Windsurf output replay advanced beyond committed Core authority".to_owned(),
            ));
        }
        ...
        let output = NativeProOutputPage {
            ...
            parser_revision: WINDSURF_OUTPUT_PARSER_REVISION.to_owned(),
        ...
        let replay = NativeProReplayPage::new_with_source_identity(
            NativeSourceIdentity::new(CaptureProvider::Windsurf.as_str(), &locator_identity),
            expected_frontier,
    ...
    let outcome = reader.outcome().ok_or(CaptureError::SystemInvariant(
        "Windsurf output replay reader completed without an outcome",
    ))?;
    if !outcome.checkpoint.terminal
        || !direct_jsonl_checkpoint_is_covered_by(authority, &outcome.checkpoint)
    {
        return Err(CaptureError::InvalidPayload(
            "Windsurf output replay outcome exceeded committed Core authority".to_owned(),
        ));
    }
    Ok(())
}

struct WindsurfOutputState {
            ...
            source_epoch: if rewrite {
                progress
                    ...
                    .ok_or(CaptureError::SystemInvariant(
                        "Windsurf output source epoch exhausted",
                    ))?
            } else {
...
fn safe_frontier(checkpoint: &DirectJsonlCheckpoint) -> Result<NativeSafeFrontier> {
    NativeSafeFrontier::new(
        WINDSURF_OUTPUT_FRONTIER_VERSION,
...
fn output_observation(
    ...
    ProOutputObservation {
        ...
        locator: OutputSourceLocator {
            version: 1,
            kind: "windsurf-jsonl-range-v1".to_owned(),
...
fn provider_sync_cursor(
    ...
    SyncCursor {
        id: stable_capture_uuid(
            &format!(
                "provider-cursor:{}:{}:{}",
                CaptureProvider::Windsurf.as_str(),
                machine_id,
                stream
            ),
...
fn retirement_publication_id(retirement: &ProviderSourceRouteRetirement) -> String {
    ...
    digest.update(b"ctx-windsurf-nativepath-route-retirement-v1\0");
    ...
    format!("windsurf-nativepath-retirement-v1:{:x}", digest.finalize())
}
