...
fn replay_outputs_or_mark_behind(
    ...
    if let Err(error) = replay_windsurf_outputs(paths, source_root, imported_at, sink) {
...
fn replay_windsurf_outputs(
    ...
    for path in paths {
        ...
        replay_windsurf_source(path, imported_at, sink, source, locator_identity, progress)?;
    }
    Ok(())
}
