...
pub(crate) fn import_qoder_nativepath_tree(
    ...
    let known_routes = known_qoder_routes(store, &request.machine_id, &configured_source_root)?;
    ...
    if live_inventory.paths.is_empty() {
        if known_routes.is_empty() {
            return Err(CaptureError::InvalidProviderTranscriptPath {
                ...
                reason: super::super::dialect::native_jsonl_missing_reason(CaptureProvider::Qoder),
            });
        }
...
fn known_qoder_routes(
    store: &Store,
    machine_id: &str,
    source_root: &Path,
) -> Result<Vec<KnownQoderRoute>> {
    let source_root = source_root.display().to_string();
    let mut routes = BTreeMap::<String, KnownQoderRoute>::new();
    for source in store.list_capture_sources()? {
        if source.descriptor.provider != CaptureProvider::Qoder
            || source.descriptor.machine_id != machine_id
            || source.descriptor.source_format.as_deref() != Some(QODER_SOURCE_FORMAT)
        ...
        if routes.insert(locator_identity, route).is_some() {
            return Err(CaptureError::SystemInvariant(
                "Qoder persisted duplicate current routes for one transcript",
            ));
        }
    }
...
fn retire_route(
    ...
    let retirement = ProviderSourceRouteRetirement {
        ...
        source_format: QODER_SOURCE_FORMAT.to_owned(),
...
fn replay_qoder_source(
    ...
    let progress_cursor = progress
        ...
        .filter(|cursor| cursor.version == QODER_OUTPUT_FRONTIER_VERSION)
        .and_then(|cursor| serde_json::from_slice::<DirectJsonlCheckpoint>(&cursor.payload).ok())
        .filter(|checkpoint| {
            checkpoint.is_supported_for(CaptureProvider::Qoder, QODER_SOURCE_FORMAT)
        });
    let can_resume = progress.as_ref().is_some_and(|progress| {
        progress.parser_revision == QODER_OUTPUT_PARSER_REVISION
    ...
    let mut output_state = QoderOutputState::new(
        output_source,
        progress,
        source_change,
        can_resume,
    ...
    while let Some(page) = reader.next_page()? {
        if !direct_jsonl_checkpoint_is_covered_by(authority, &page.next_checkpoint) {
            return Err(CaptureError::InvalidPayload(
                "Qoder output replay advanced beyond committed Core authority".to_owned(),
            ));
        }
        ...
        let output = NativeProOutputPage {
            ...
            parser_revision: QODER_OUTPUT_PARSER_REVISION.to_owned(),
        ...
        let replay = NativeProReplayPage::new_with_source_identity(
            NativeSourceIdentity::new(CaptureProvider::Qoder.as_str(), &locator_identity),
            expected_frontier,
    ...
    let outcome = reader.outcome().ok_or(CaptureError::SystemInvariant(
        "Qoder output replay reader completed without an outcome",
    ))?;
    if !outcome.checkpoint.terminal
        || !direct_jsonl_checkpoint_is_covered_by(authority, &outcome.checkpoint)
    {
        return Err(CaptureError::InvalidPayload(
            "Qoder output replay outcome exceeded committed Core authority".to_owned(),
        ));
    }
    Ok(())
}

struct QoderOutputState {
            ...
            source_epoch: if rewrite {
                progress
                    ...
                    .ok_or(CaptureError::SystemInvariant(
                        "Qoder output source epoch exhausted",
                    ))?
            } else {
...
fn output_observation(
    ...
    ProOutputObservation {
        ...
        locator: OutputSourceLocator {
            version: 1,
            kind: "qoder-jsonl-range-v1".to_owned(),
...
fn retirement_publication_id(retirement: &ProviderSourceRouteRetirement) -> String {
    ...
    digest.update(b"ctx-qoder-nativepath-route-retirement-v1\0");
    ...
    format!("qoder-nativepath-retirement-v1:{:x}", digest.finalize())
}
