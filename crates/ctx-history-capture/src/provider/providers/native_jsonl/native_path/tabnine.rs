...
pub(crate) fn import_tabnine_nativepath_tree(
    store: &mut Store,
    ...
    let known_routes = known_tabnine_routes(store, &request.machine_id, &configured_source_root)?;
...
fn known_tabnine_routes(
    store: &Store,
    machine_id: &str,
    source_root: &Path,
) -> Result<Vec<KnownTabnineRoute>> {
    let source_root = source_root.display().to_string();
    let mut routes = BTreeMap::<String, KnownTabnineRoute>::new();
    for source in store.list_capture_sources()? {
        if source.descriptor.provider != CaptureProvider::Tabnine
            || source.descriptor.machine_id != machine_id
            || source.descriptor.source_format.as_deref() != Some(TABNINE_CLI_SOURCE_FORMAT)
        ...
        if routes.insert(locator_identity, route).is_some() {
            return Err(CaptureError::SystemInvariant(
                "Tabnine persisted duplicate current routes for one transcript",
            ));
        }
    }
...
fn retire_route(
    ...
    let retirement = ProviderSourceRouteRetirement {
        ...
        source_format: TABNINE_CLI_SOURCE_FORMAT.to_owned(),
...
fn replay_tabnine_source(
    ...
    let progress_cursor = progress
        ...
        .filter(|cursor| cursor.version == TABNINE_OUTPUT_FRONTIER_VERSION)
        .and_then(|cursor| serde_json::from_slice::<DirectJsonlCheckpoint>(&cursor.payload).ok())
        .filter(|checkpoint| {
            checkpoint.is_supported_for(CaptureProvider::Tabnine, TABNINE_CLI_SOURCE_FORMAT)
        });
    let can_resume = progress.as_ref().is_some_and(|progress| {
        progress.parser_revision == TABNINE_OUTPUT_PARSER_REVISION
    ...
    while let Some(page) = reader.next_page()? {
        ...
        let output = NativeProOutputPage {
            ...
            parser_revision: TABNINE_OUTPUT_PARSER_REVISION.to_owned(),
        ...
        let replay = NativeProReplayPage::new_with_source_identity(
            NativeSourceIdentity::new(CaptureProvider::Tabnine.as_str(), &locator_identity),
            expected_frontier,
            ...
            source_epoch: if rewrite {
                progress
                    ...
                    .ok_or(CaptureError::SystemInvariant(
                        "Tabnine output source epoch exhausted",
                    ))?
            } else {
...
fn safe_frontier(checkpoint: &DirectJsonlCheckpoint) -> Result<NativeSafeFrontier> {
    NativeSafeFrontier::new(
        TABNINE_OUTPUT_FRONTIER_VERSION,
...
fn output_observation(
    ...
    ProOutputObservation {
        ...
        locator: OutputSourceLocator {
            version: 1,
            kind: "tabnine-jsonl-range-v1".to_owned(),
...
fn retirement_publication_id(retirement: &ProviderSourceRouteRetirement) -> String {
    ...
    digest.update(b"ctx-tabnine-nativepath-route-retirement-v1\0");
    ...
    format!("tabnine-nativepath-retirement-v1:{:x}", digest.finalize())
}
