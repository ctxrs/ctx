...
pub(super) fn map_result_capture_error(
    request: &ResultContentRequest,
    cause: CaptureError,
) -> CompleteContentError {
    match cause {
        CaptureError::Io(cause) => {
            let kind = if cause.kind() == std::io::ErrorKind::NotFound {
                CompleteContentErrorKind::SourceMissing
            } else {
                CompleteContentErrorKind::SourceUnreadable
            };
            result_error(request, kind)
        }
        CaptureError::SourceChangedDuringCapture => {
            result_error(request, CompleteContentErrorKind::SourceChanged)
        }
        CaptureError::InvalidProviderTranscriptPath { .. } => {
            result_error(request, CompleteContentErrorKind::SourceUnreadable)
        }
        CaptureError::Sqlite(cause) => {
            if let rusqlite::Error::SqliteFailure(failure, _) = &cause {
                ...
                return result_error(request, kind);
            }
            let kind = if matches!(
                cause,
                rusqlite::Error::FromSqlConversionFailure(..)
        ...
        _ => result_error(request, CompleteContentErrorKind::ContentVerificationFailed),
    }
}
