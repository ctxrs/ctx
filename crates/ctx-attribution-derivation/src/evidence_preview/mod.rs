...
pub use ctx_attribution_model::{
    EvidencePreview, EvidencePreviewModel, MAX_EVIDENCE_PREVIEW_CITATIONS,
    MAX_EVIDENCE_PREVIEW_EXCERPT_BYTES,
};
use ctx_history_core::{
    CORE_CONTENT_POLICY_REVISION, CORE_NORMALIZATION_REVISION, CORE_RECORD_VERSION, CoreRecord,
};
pub use ctx_repository_evidence::RepositoryFileInvocationKind;
use ctx_repository_evidence::{
    CoreRepositoryEvidenceAdapter, RepositoryBinding, RepositoryEvaluation,
...
#[derive(Debug, Clone)]
pub struct VerifiedEvidenceRecord<'a> {
...
impl<'a> VerifiedEvidenceRecord<'a> {
    ...
    pub fn new(
        ...
        let cited_digest =
            validated_evidence_record(numbered, hydrated_core_generation_id, record)?;

        // Evidence hydration selects at most three records and is not a complete source stream.
        // Reset the private adapter for every immutable record so partial hydration cannot create
        // cross-record linkage or synthesize repository authority from omitted events.
        let mut adapter = CoreRepositoryEvidenceAdapter::default();
        adapter.begin_source();
        let repository_evaluation = adapter.evaluate_record(record, cited_digest);
        Some(Self {
            numbered,
            record,
    ...
    #[cfg(test)]
    fn new_with_repository_evaluation(
        numbered: &'a NumberedEvidence,
        hydrated_core_generation_id: &str,
        record: &'a CoreRecord,
        repository_evaluation: RepositoryEvaluation,
...
fn validated_evidence_record<'a>(
    numbered: &'a NumberedEvidence,
    hydrated_core_generation_id: &str,
    record: &CoreRecord,
) -> Option<&'a str> {
    ...
    let encoded = record.encode_stored().ok()?;
...
/// or Core data.
#[must_use]
pub fn project_evidence_previews(
    result: &BlameResult,
    ...
    for numbered in citations {
        ...
        let Some(mut preview) = project_one(
            path,
            repository,
            candidate.record,
            &candidate.repository_evaluation,
        ) else {
            continue;
        };
...
fn citation_matches_record(citation: &EvidenceCitation, record: &CoreRecord) -> bool {
    citation.source.validate_contract().is_ok()
        && citation.source.exact_descriptor_eq(&record.source)
        && citation.session_id == record.session_id
        && citation.event_id == record.event_id
        && citation.event_sequence == record.event_sequence
}

fn validated_current_core_contract(record: &CoreRecord) -> bool {
    record.validate_contract().is_ok()
        && record.record_version == CORE_RECORD_VERSION
...
fn project_one(
    ...
    let binding = exact_repository_binding(repository, repository_evaluation)?;
    let target_invocation = exact_target_invocation(target, binding, repository_evaluation)?;
    let invocation = target_invocation.invocation;
    if invocation.repository_binding_id != binding.binding_id
        || invocation_ranges_overlap(target_invocation.index, invocation, repository_evaluation)
    {
        return None;
    }
    ...
    Some(EvidencePreview {
        ...
        event_occurred_at_ms: record.occurred_at_unix_ms,
...
fn exact_target_invocation<'a>(
    ...
    absolute_style?;
    let mut selected_matches = repository_evaluation
        .repository_file_invocation_evidence
        .iter()
        .enumerate()
    ...
    if selected_matches.next().is_some()
        || absolute_invocation_is_ambiguous(
            target,
            matched,
            selected_binding,
            repository_evaluation,
...
fn absolute_invocation_is_ambiguous(
    ...
    repository_evaluation
        ...
        .any(|(_, invocation)| {
            let Some(binding) =
                unique_binding(&invocation.repository_binding_id, repository_evaluation)
            else {
                return true;
            };
