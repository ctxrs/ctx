...
usage() {
  cat >&2 <<'USAGE'
Usage: scripts/smoke-daemon-semantic-release.sh --runtime-archive PATH --runtime-platform PLATFORM [--ctx PATH] [--data-root DIR] [--timeout-seconds N] [--keep-root]

Native release smoke for opt-in daemon + semantic search. The smoke creates an
isolated ctx data root, installs the packaged ONNX Runtime 1.27.0 sidecar under
an isolated CTX_RUNTIME_DIR, imports a tiny custom-history fixture, enables
daemon and semantic search in that root only, runs the daemon, verifies strict
semantic search can find the fixture, and stops the daemon process it started.
When --data-root is provided, it is the parent for a fresh unique run root;
--keep-root preserves that child for inspection.
USAGE
}
...
if [[ -z "${runtime_archive}" ]]; then
  echo "error: --runtime-archive is required" >&2
  exit 2
fi
...
ctx_version="$(${ctx_source} --version | awk 'NR == 1 { print $2 }')"
[[ -n "${ctx_version}" ]] || {
...
query="release smoke semantic opal maple lantern"
...
daemon_status_matches() {
  python3 -I - "${daemon_status_json}" "${daemon_pid}" <<'PY'
import json
import sys
...
if pid is not None and (isinstance(pid, bool) or not isinstance(pid, int) or pid != expected_pid):
    ...
    raise SystemExit(2)
if daemon.get("status") == "running" and daemon.get("running") is True and pid == expected_pid:
    raise SystemExit(0)
raise SystemExit(1)
PY
}
...
search_json_matches() {
  python3 -I - "${search_json}" "${marker}" "${embedding_model}" <<'PY'
import json
import sys

path, marker, expected_model = sys.argv[1:]
try:
    with open(path, encoding="utf-8") as source:
        payload = json.load(source)
except (OSError, UnicodeError, json.JSONDecodeError):
