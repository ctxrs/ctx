...
if command -v rg >/dev/null 2>&1; then
  if rg -n -i 'dashboard|shim|shims|pull request|pr evidence|ctx pr|ctx publish|ctx evidence|hosted|ADE|automatic summar|\bMVP\b|recover prior decisions|ctx remembers everything|privacy-first|ctx context|ctx update|update checks|auto-update|update-state|auto_update|CTX_UPDATE|release manifest|--until|ctx list --provider|ctx list --repo|ctx list --since' \
    README.md SECURITY.md docs skills; then
